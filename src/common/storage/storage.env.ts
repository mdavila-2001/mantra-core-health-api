...
export const storageEnvSchema = Joi.object({
  FILE_STORAGE_ADAPTER: Joi.string().valid(...FILE_STORAGE_ADAPTERS).default('local'),
  FILE_STORAGE_LOCAL_DIR: Joi.string().default('./storage/uploads'),
  FILE_STORAGE_MAX_SIZE_BYTES: Joi.number().integer().min(1).default(DEFAULT_MAX_UPLOAD_BYTES),
...
export function loadStorageEnv(): StorageEnv {
  return {
    adapter: (process.env.FILE_STORAGE_ADAPTER ?? 'local') as FileStorageAdapterName,
    localDir: process.env.FILE_STORAGE_LOCAL_DIR ?? './storage/uploads',
    maxSizeBytes: Number(process.env.FILE_STORAGE_MAX_SIZE_BYTES ?? DEFAULT_MAX_UPLOAD_BYTES),
    s3: {
