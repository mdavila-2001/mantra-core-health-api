...
function build() {
  ...
  const service = new ProcessingLegalBasesService(em as any, legalBasesRepo as any, logger as any);
...
describe('ProcessingLegalBasesService', () => {
  it('version (UC-07-06) inserts a new active version without a prior one', async () => {
    ...
    const created = { id: 'lb1', processingPurposeId: 'pp1', statusConceptId: CONS.LEGAL_BASIS_ACTIVE, createdAt: new Date() };
    d.legalBasesRepo.create.mockReturnValue(created);

    const res = await d.service.version({ processingPurposeId: 'pp1' } as any, actor);
  ...
  it('version (UC-07-06) supersedes the current version', async () => {
    const d = build();
    const current = { id: 'lb0', statusConceptId: CONS.LEGAL_BASIS_ACTIVE, updatedAt: new Date() };
    d.legalBasesRepo.findCurrentVersion.mockResolvedValue(current);
    d.legalBasesRepo.create.mockReturnValue({ id: 'lb1', processingPurposeId: 'pp1', statusConceptId: CONS.LEGAL_BASIS_ACTIVE, createdAt: new Date() });
