...
function build() {
  ...
  const objectionsRepo = { findOpenByPurpose: mockFn(), findById: mockFn(), create: mockFn() };
  const restrictionsRepo = { create: mockFn(), findActiveByPatient: mockFn().mockResolvedValue([]) };
  const eventsRepo = { record: mockFn() };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };

  const service = new PatientObjectionsService(
    em as any,
    objectionsRepo as any,
...
describe('PatientObjectionsService', () => {
  describe('raise (UC-07-03)', () => {
    it('rejects when an open objection already exists', async () => {
      ...
      await expect(
        d.service.raise({ patientProfileId: 'p1', processingPurposeId: 'pp1' } as any, actor),
    ...
    it('raises an objection without restriction', async () => {
      ...
      const obj = { id: 'o1', patientProfileId: 'p1', statusConceptId: CONS.OBJECTION_STATUS_RAISED, createdAt: new Date() };
      d.objectionsRepo.create.mockReturnValue(obj);

      const res = await d.service.raise({ patientProfileId: 'p1', processingPurposeId: 'pp1' } as any, actor);
      ...
      expect(d.eventsRepo.record).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ eventTypeConceptId: CONS.EVENT_OBJECTION_RAISED }),
      );
    });
    ...
    it('materializes a restriction when applyRestriction is set (include UC-07-07)', async () => {
      const d = build();
      d.objectionsRepo.findOpenByPurpose.mockResolvedValue(null);
      d.objectionsRepo.create.mockReturnValue({ id: 'o1', patientProfileId: 'p1', statusConceptId: CONS.OBJECTION_STATUS_RAISED, createdAt: new Date() });
      d.restrictionsRepo.create.mockReturnValue({ id: 'r1' });

      const res = await d.service.raise(
        { patientProfileId: 'p1', processingPurposeId: 'pp1', applyRestriction: true } as any,
        actor,
      );
  ...
  describe('resolve (UC-07-12)', () => {
    it('throws when the objection does not exist', async () => {
      ...
      await expect(d.service.resolve('missing', { resolution: 'UPHELD' } as any, actor)).rejects.toBeInstanceOf(
    ...
    it('rejects resolving an objection that is not open', async () => {
      const d = build();
      d.objectionsRepo.findById.mockResolvedValue({ id: 'o1', statusConceptId: CONS.OBJECTION_STATUS_RESOLVED });
      await expect(d.service.resolve('o1', { resolution: 'UPHELD' } as any, actor)).rejects.toBeInstanceOf(
        ConflictException,
      );
    });
    ...
    it('revokes active restrictions when the objection is rejected', async () => {
      const d = build();
      const obj: any = { id: 'o1', patientProfileId: 'p1', statusConceptId: CONS.OBJECTION_STATUS_RAISED, updatedAt: new Date() };
      ...
      const res = await d.service.resolve('o1', { resolution: 'REJECTED' } as any, actor);
