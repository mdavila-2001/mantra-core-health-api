...
function build() {
  ...
  const authRepo = { create: mockFn(), findById: mockFn(), findExpirable: mockFn().mockResolvedValue([]) };
  const eventsRepo = { record: mockFn() };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };

  const service = new HipaaAuthorizationsService(em as any, authRepo as any, eventsRepo as any, logger as any);
...
describe('HipaaAuthorizationsService', () => {
  describe('grant (UC-07-04)', () => {
    it('rejects DATE expiration without expiresAt', async () => {
      ...
      await expect(
        d.service.grant(
          { patientProfileId: 'p1', processingPurposeId: 'pp1', expirationType: 'DATE' } as any,
          actor,
        ),
    ...
    it('grants an authorization and records the authorized event', async () => {
      const d = build();
      const created = { id: 'h1', patientProfileId: 'p1', statusConceptId: CONS.HIPAA_ACTIVE, createdAt: new Date() };
      ...
      expect(res).toEqual({ id: 'h1', patientProfileId: 'p1', status: CONS.HIPAA_ACTIVE, createdAt: created.createdAt });
  ...
  describe('revoke (UC-07-05)', () => {
    it('throws when the authorization does not exist', async () => {
      ...
      await expect(d.service.revoke('missing', actor)).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('rejects revoking an authorization that is not active', async () => {
      const d = build();
      d.authRepo.findById.mockResolvedValue({ id: 'h1', statusConceptId: CONS.HIPAA_REVOKED });
      await expect(d.service.revoke('h1', actor)).rejects.toBeInstanceOf(ConflictException);
    });
