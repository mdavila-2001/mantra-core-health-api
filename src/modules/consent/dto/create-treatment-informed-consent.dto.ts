...
export class CreateTreatmentInformedConsentDto {
  ...
  @ApiProperty({ description: 'Encuentro clínico abierto (encounter id)', format: 'uuid' })
  @IsUUID()
  encounterId!: string;

  @ApiProperty({ description: 'Decisión del paciente', enum: ['ACCEPTED', 'DECLINED'] })
  @IsIn(['ACCEPTED', 'DECLINED'])
  decision!: TreatmentDecision;

  @ApiPropertyOptional({ description: 'Código de procedimiento (concept id)', format: 'uuid' })
  @IsOptional()
  @IsUUID()
  procedureCodeConceptId?: string;

  @ApiPropertyOptional({ description: 'Versión del material informativo vigente' })
  @IsOptional()
  @IsString()
  @MaxLength(100)
  informationVersion?: string;

  @ApiPropertyOptional({ description: 'Intérprete presente (user id)', format: 'uuid' })
  @IsOptional()
  @IsUUID()
