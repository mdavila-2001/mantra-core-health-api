...
function build() {
  ...
  const unitsRepo = { findById: mockFn(), findBySite: mockFn().mockResolvedValue([]), create: mockFn() };
  const spacesRepo = { findById: mockFn(), findBySite: mockFn().mockResolvedValue([]), create: mockFn() };
  ...
  return { service, tx, practicesRepo, sitesRepo, unitsRepo, spacesRepo, servicesRepo };
}
...
describe('ClinicalStructureService', () => {
  describe('createClinicalUnit (UC-14-04)', () => {
    it('throws when the site is missing', async () => {
      ...
      await expect(
        d.service.createClinicalUnit('s1', { code: 'U-1', name: 'Unit' } as any, actor),
    ...
    it('rejects a parent unit outside the site', async () => {
      ...
      d.unitsRepo.findById.mockResolvedValue({ id: 'u0', practiceSiteId: 'other' });
      await expect(
        d.service.createClinicalUnit('s1', { code: 'U-1', name: 'Unit', parentUnitId: 'u0' } as any, actor),
    ...
    it('creates an active unit', async () => {
      const d = build();
      d.sitesRepo.findById.mockResolvedValue(activeSite);
      const created = { id: 'u1', practiceSiteId: 's1', code: 'U-1', statusConceptId: PRAC.UNIT_ACTIVE, createdAt: new Date() };
      d.unitsRepo.create.mockReturnValue(created);
      const res = await d.service.createClinicalUnit('s1', { code: 'U-1', name: 'Unit' } as any, actor);
  ...
  describe('createCareSpace (UC-14-05)', () => {
    it('rejects when the site is not active', async () => {
      const d = build();
      d.sitesRepo.findById.mockResolvedValue({ id: 's1', statusConceptId: 'other' });
      await expect(
        d.service.createCareSpace('s1', { code: 'C-1', name: 'Room' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });
    ...
    it('creates an available space', async () => {
      ...
      const res = await d.service.createCareSpace('s1', { code: 'C-1', name: 'Room' } as any, actor);
  ...
  describe('publishHealthcareService (UC-14-06)', () => {
    it('throws when the practice is missing', async () => {
      ...
      await expect(d.service.publishHealthcareService('p1', {} as any, actor)).rejects.toBeInstanceOf(
    ...
    it('publishes an active service', async () => {
      ...
      const created = { id: 'hs1', practiceId: 'p1', statusConceptId: PRAC.SERVICE_ACTIVE, createdAt: new Date() };
      d.servicesRepo.create.mockReturnValue(created);
      const res = await d.service.publishHealthcareService('p1', {} as any, actor);
