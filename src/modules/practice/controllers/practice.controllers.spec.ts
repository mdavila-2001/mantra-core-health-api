...
describe('PracticesController', () => {
  ...
  function build() {
    const sitesService = {
      createPractice: mockFn(),
      createSite: mockFn(),
      decommissionSite: mockFn(),
    };
  ...
  it('delegates createPractice (bootstrap)', async () => {
    ...
    await d.controller.createPractice(dto, actor);
  ...
  it('delegates createSite (UC-14-01)', async () => {
    ...
    await d.controller.createSite('p1', dto, actor);
  ...
  it('delegates decommissionSite (UC-14-12)', async () => {
    ...
    expect(d.sitesService.decommissionSite).toHaveBeenCalledWith(
      'p1',
      's1',
      actor,
    );
  ...
  it('delegates createAccreditation (UC-14-02)', async () => {
    ...
    await d.controller.createAccreditation('p1', dto, actor);
    expect(d.accreditationsService.create).toHaveBeenCalledWith(
      'p1',
      dto,
      actor,
    );
  ...
  it('delegates publishHealthcareService (UC-14-06)', async () => {
    const d = build();
    const dto = {};
    await d.controller.publishHealthcareService('p1', dto, actor);
    expect(d.structureService.publishHealthcareService).toHaveBeenCalledWith(
      'p1',
      dto,
      actor,
    );
  ...
  it('delegates upsertSetting (UC-14-07)', async () => {
    ...
    await d.controller.upsertSetting('p1', 'k', dto, actor);
    expect(d.settingsService.upsert).toHaveBeenCalledWith(
      'p1',
      'k',
      dto,
      actor,
  ...
  it('delegates assignRole (UC-14-08)', async () => {
    ...
    await d.controller.assignRole('p1', dto, actor);
    expect(d.workforceService.assignRole).toHaveBeenCalledWith(
      'p1',
      dto,
      actor,
    );
  ...
  it('delegates createInventoryItem (UC-14-10)', async () => {
    ...
    await d.controller.createInventoryItem('p1', dto, actor);
    expect(d.inventoryService.createItem).toHaveBeenCalledWith(
      'p1',
      dto,
      actor,
    );
...
describe('SitesController', () => {
  it('delegates clinical unit and care space (UC-14-04/05)', async () => {
    ...
    await controller.createCareSpace('s1', { code: 'C', name: 'N' }, actor);
    expect(structureService.createCareSpace).toHaveBeenCalledWith(
      's1',
      { code: 'C', name: 'N' },
      actor,
    );
...
describe('AccreditationsController', () => {
  it('delegates verify (UC-14-03)', async () => {
    ...
    expect(accreditationsService.verify).toHaveBeenCalledWith(
      'a1',
      { decision: 'VERIFIED' },
      actor,
    );
...
describe('RoleAssignmentsController', () => {
  it('delegates attachSupport (UC-14-09)', async () => {
    ...
    await controller.attachSupport('r1', { supportProfileId: 'sp1' }, actor);
    expect(workforceService.attachSupport).toHaveBeenCalledWith(
      'r1',
...
describe('InventoryItemsController', () => {
  it('delegates recordMovement (UC-14-11)', async () => {
    ...
    await controller.recordMovement(
      'it1',
      { direction: 'IN', quantity: 1 } as any,
      actor,
    );
