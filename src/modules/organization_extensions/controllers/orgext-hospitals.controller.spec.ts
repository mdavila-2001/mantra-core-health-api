...
describe('OrgextHospitalsController', () => {
  it('delegates specialize (UC-22-01)', async () => {
    ...
    await expect(d.controller.specialize(dto as any, actor)).resolves.toEqual({ id: 'h1' });
  ...
  it('delegates activate (UC-22-02)', async () => {
    ...
    await d.controller.activate('h1', dto as any, actor);
  ...
  it('delegates addServiceLine (UC-22-03)', async () => {
    const d = build();
    const dto = {};
    await d.controller.addServiceLine('h1', dto as any, actor);
    expect(d.hospitalsService.addServiceLine).toHaveBeenCalledWith('h1', dto, actor);
  });

  it('delegates retireServiceLine (UC-22-04)', async () => {
    const d = build();
    await d.controller.retireServiceLine('h1', 'l1', actor);
    expect(d.hospitalsService.retireServiceLine).toHaveBeenCalledWith('h1', 'l1', actor);
  });
});
