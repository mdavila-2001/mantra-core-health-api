 ...
 */
export const { seeds: FORMS_CONCEPT_SEEDS, ids: FORMS } = defineModuleConcepts('forms', {
  // --- Ciclo de vida del set de definiciones (field_definition_sets.status) ---
  SET_STATUS_DRAFT: { code: 'FORMS_SET_DRAFT', display: 'Definition set draft' },
  SET_STATUS_ACTIVE: { code: 'FORMS_SET_ACTIVE', display: 'Definition set active' },

  // --- Publicación de versión (field_definition_set_versions.publication_status) ---
  PUB_DRAFT: { code: 'FORMS_PUB_DRAFT', display: 'Version publication draft' },
  PUB_PUBLISHED: { code: 'FORMS_PUB_PUBLISHED', display: 'Version published' },
  COMPAT_BACKWARD: { code: 'FORMS_COMPAT_BACKWARD', display: 'Backward compatible' },

  // --- Dominio destino por defecto (field_definition_sets.target_domain) ---
  TARGET_DOMAIN_GENERIC: { code: 'FORMS_TARGET_DOMAIN_GENERIC', display: 'Generic target domain' },

  // --- Estado de definición de campo / asignación (state_concept_id) ---
  FIELD_ACTIVE: { code: 'FORMS_FIELD_ACTIVE', display: 'Field definition active' },
  ASSIGNMENT_ACTIVE: { code: 'FORMS_ASSIGNMENT_ACTIVE', display: 'Field assignment active' },

  // --- Reglas de validación (field_validation_rules) ---
  RULE_TYPE_REQUIRED: { code: 'FORMS_RULE_REQUIRED', display: 'Required rule' },
  ...
  SEVERITY_WARNING: { code: 'FORMS_SEVERITY_WARNING', display: 'Warning severity' },

  // --- Operadores y comportamientos de dependencia (field_dependencies) ---
  OP_EQUALS: { code: 'FORMS_OP_EQUALS', display: 'Equals operator' },
  OP_NOT_EQUALS: { code: 'FORMS_OP_NOT_EQUALS', display: 'Not-equals operator' },
  ...
  BEHAVIOR_REQUIRE: { code: 'FORMS_BEHAVIOR_REQUIRE', display: 'Require behavior' },

  // --- Ciclo de vida de la instancia de formulario (form_instances.state) ---
  INSTANCE_OPEN: { code: 'FORMS_INSTANCE_OPEN', display: 'Form instance open' },
  INSTANCE_CLOSED: { code: 'FORMS_INSTANCE_CLOSED', display: 'Form instance closed' },

  // --- Origen y estado de los valores capturados (field_values) ---
  SOURCE_INTERNAL: { code: 'FORMS_SOURCE_INTERNAL', display: 'Internally captured value' },
  SOURCE_EXTERNAL: { code: 'FORMS_SOURCE_EXTERNAL', display: 'Externally imported value' },
  VALUE_FINAL: { code: 'FORMS_VALUE_FINAL', display: 'Final value' },
  VALUE_PRELIMINARY: { code: 'FORMS_VALUE_PRELIMINARY', display: 'Preliminary value' },
  VALUE_CORRECTED: { code: 'FORMS_VALUE_CORRECTED', display: 'Corrected value' },
  VALUE_SUPERSEDED: { code: 'FORMS_VALUE_SUPERSEDED', display: 'Superseded value' },
  ...
  REASON_CORRECTION: { code: 'FORMS_REASON_CORRECTION', display: 'Data entry correction' },

  // --- Tipo de recurso destino por defecto (form_instances / field_values) ---
  RESOURCE_TYPE_PATIENT: { code: 'FORMS_RESOURCE_PATIENT', display: 'Patient resource' },

  // --- Enmascarado y estado de reglas de acceso (field_value_access_rules) ---
  ACCESS_RULE_ACTIVE: { code: 'FORMS_ACCESS_RULE_ACTIVE', display: 'Access rule active' },
  ...
  MIGRATION_PENDING: { code: 'FORMS_MIGRATION_PENDING', display: 'Migration pending' },
  MIGRATION_RUNNING: { code: 'FORMS_MIGRATION_RUNNING', display: 'Migration running' },
  MIGRATION_COMPLETED: { code: 'FORMS_MIGRATION_COMPLETED', display: 'Migration completed' },
  MIGRATION_TYPE_TRANSFORM: { code: 'FORMS_MIGRATION_TRANSFORM', display: 'Transformation migration' },

  // --- Verificación de procedencia importada (field_value_provenance) ---
  VERIF_UNVERIFIED: { code: 'FORMS_VERIF_UNVERIFIED', display: 'Provenance unverified' },
  VERIF_VERIFIED: { code: 'FORMS_VERIF_VERIFIED', display: 'Provenance verified' },
});

/** Comportamientos de dependencia expuestos por el DTO → concept id. */
export const DEPENDENCY_BEHAVIOR_BY_CODE: Record<'SHOW' | 'HIDE' | 'REQUIRE', string> = {
