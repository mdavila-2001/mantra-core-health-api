...
function build() {
  ...
  const valuesRepo = { findPreliminaryByInstance: mockFn().mockResolvedValue([]) };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };
  const service = new FormsInstancesService(em as any, instancesRepo as any, valuesRepo as any, logger as any);
...
describe('FormsInstancesService', () => {
  describe('openInstance (UC-09-07)', () => {
    it('opens a new instance for a resource', async () => {
      ...
      const res = await d.service.openInstance({ resourceId: 'r1' } as any, actor);
      expect(res).toEqual({ id: 'i1', schemaVersion: 1, state: FORMS.INSTANCE_OPEN });
    });

    it('rejects a duplicate instance', async () => {
      const d = build();
      d.instancesRepo.findByResourceAndVersion.mockResolvedValue({ id: 'i0' });
      await expect(d.service.openInstance({ resourceId: 'r1' } as any, actor)).rejects.toBeInstanceOf(
        ConflictException,
      );
    });
  });
  ...
  describe('closeInstance (UC-09-11)', () => {
    it('closes an open instance and finalizes preliminary values', async () => {
      const d = build();
      const instance = { id: 'i1', stateConceptId: FORMS.INSTANCE_OPEN, updatedAt: new Date() };
      const prelim = { id: 'v1', valueStatusConceptId: FORMS.VALUE_PRELIMINARY, updatedAt: new Date() };
    ...
    it('rejects closing an instance that is not open', async () => {
      const d = build();
      d.instancesRepo.findById.mockResolvedValue({ id: 'i1', stateConceptId: FORMS.INSTANCE_CLOSED });
