...
function build() {
  ...
  const subledgerRepo = {
    ...
    createOpenItem: mockFn((_em: any, d: any) => ({ id: 'oi1', statusConceptId: d.statusConceptId, outstandingAmount: d.outstandingAmount })),
    findClearingByNumber: mockFn().mockResolvedValue(null),
    createClearingDocument: mockFn((_em: any, d: any) => ({ id: 'cd1', clearingNumber: d.clearingNumber })),
  ...
  const service = new SubledgerService(em as any, subledgerRepo as any, posting as any, logger as any);
...
describe('SubledgerService', () => {
  describe('createOpenItem (UC-16-08)', () => {
    it('lanza 404 si el subledger no existe', async () => {
      ...
      await expect(
        d.service.createOpenItem(
          { tenantId: 't', subledgerAccountId: 's', ledgerEntryId: 'e', documentType: 'INVOICE', originalAmount: '100.00' } as any,
          actor,
        ),
    ...
    it('crea la partida abierta con estado ABIERTO', async () => {
      const d = build();
      d.subledgerRepo.findSubledgerById.mockResolvedValue({ id: 's', reconciliationAccountId: 'rec' });
  ...
  describe('clearOpenItems (UC-16-09)', () => {
    ...
    it('lanza 404 si la partida no existe', async () => {
      ...
      await expect(d.service.clearOpenItems(dto as any, actor)).rejects.toBeInstanceOf(
    ...
    it('compensa la partida y postea el asiento de banco', async () => {
      ...
      d.subledgerRepo.findSubledgerById.mockResolvedValue({ id: 's', reconciliationAccountId: 'rec' });
      const res = await d.service.clearOpenItems(dto as any, actor);
