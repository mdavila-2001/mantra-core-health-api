...
function build() {
  ...
  const accrualRepo = {
    ...
    createObject: mockFn((_em: any, d: any) => ({ id: 'ao1', objectNumber: d.objectNumber, statusConceptId: d.statusConceptId })),
  ...
  const service = new AccrualService(em as any, accrualRepo as any, posting as any, logger as any);
...
describe('AccrualService', () => {
  describe('createAccrualObject (UC-16-06)', () => {
    it('crea objeto y cronograma cuando sum(planned)=total', async () => {
      const d = build();
      const res = await d.service.createAccrualObject(createDto as any, actor);
    ...
    it('rechaza (422) si el cronograma no suma el total', async () => {
      ...
      await expect(d.service.createAccrualObject(bad as any, actor)).rejects.toBeInstanceOf(
    ...
    it('rechaza (409) número de objeto duplicado', async () => {
      ...
      await expect(d.service.createAccrualObject(createDto as any, actor)).rejects.toBeInstanceOf(
        ConflictException,
      );
    });
  });
  ...
  describe('runAccruals (UC-16-07)', () => {
    const runDto = { accrualObjectId: 'ao1', fiscalPeriodId: 'fp1', practiceId: 'p1', postingDate: '2026-01-31' };

    it('lanza 404 si el objeto no existe', async () => {
      const d = build();
      d.accrualRepo.findObjectById.mockResolvedValue(null);
      await expect(d.service.runAccruals(runDto as any, actor)).rejects.toBeInstanceOf(
    ...
    it('rechaza si no hay líneas pendientes', async () => {
      const d = build();
      d.accrualRepo.findObjectById.mockResolvedValue({ id: 'ao1', expenseAccountId: 'e', accrualAccountId: 'a' });
    ...
    it('postea un asiento balanceado por línea pendiente', async () => {
      ...
      const line = { id: 'sl1', plannedAmount: '100.00', updatedAt: new Date() };
      d.accrualRepo.pendingLinesForPeriod.mockResolvedValue([line]);
      const res = await d.service.runAccruals(runDto as any, actor);
