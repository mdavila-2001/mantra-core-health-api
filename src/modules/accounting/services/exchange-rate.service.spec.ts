...
function build() {
  ...
  const ratesRepo = {
    findForPair: mockFn(),
    create: mockFn((_em: any, d: any) => ({ id: 'fx1', rate: d.rate, validOn: d.validOn })),
  };
  const logger = { setContext: mockFn(), info: mockFn() };
  const service = new ExchangeRateService(em as any, ratesRepo as any, logger as any);
...
describe('ExchangeRateService (UC-16-14)', () => {
  it('crea una tasa nueva cuando no existe', async () => {
    ...
    const res = await d.service.registerRate(dto as any, actor);
  ...
  it('actualiza la tasa existente (upsert)', async () => {
    const d = build();
    const existing = { id: 'fx1', rate: '3.50', validOn: new Date('2026-01-31'), updatedAt: new Date() };
