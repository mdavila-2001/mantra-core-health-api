 ...
 */
export const { seeds: INTEGRATIONS_CONCEPT_SEEDS, ids: INTEG } = defineModuleConcepts(
  'integrations',
  {
    ...
    PROVIDER_ACTIVE: { code: 'INTEG_PROVIDER_ACTIVE', display: 'Provider active' },
    PROVIDER_INACTIVE: { code: 'INTEG_PROVIDER_INACTIVE', display: 'Provider inactive' },

    // --- Tipo de proveedor (external_providers.provider_type_concept_id) ---
    PROVIDER_TYPE_LAB: { code: 'INTEG_PROVIDER_TYPE_LAB', display: 'Laboratory provider' },
    PROVIDER_TYPE_INSURER: { code: 'INTEG_PROVIDER_TYPE_INSURER', display: 'Insurer provider' },
    PROVIDER_TYPE_GOV: { code: 'INTEG_PROVIDER_TYPE_GOV', display: 'Government provider' },
    PROVIDER_TYPE_GENERIC: { code: 'INTEG_PROVIDER_TYPE_GENERIC', display: 'Generic provider' },

    // --- Tipo de autenticación (external_providers.auth_type_concept_id) ---
    AUTH_OAUTH2: { code: 'INTEG_AUTH_OAUTH2', display: 'OAuth2 authentication' },
    AUTH_API_KEY: { code: 'INTEG_AUTH_API_KEY', display: 'API key authentication' },
    ...
    CONN_PAUSED: { code: 'INTEG_CONN_PAUSED', display: 'Connection paused (circuit open)' },
    ...
    ENV_PRODUCTION: { code: 'INTEG_ENV_PRODUCTION', display: 'Production environment' },
    ...
    SECRET_OAUTH_TOKEN: { code: 'INTEG_SECRET_OAUTH_TOKEN', display: 'OAuth token secret' },
    ...
    ENDPOINT_PUBLISHED: { code: 'INTEG_ENDPOINT_PUBLISHED', display: 'Endpoint published' },
    ENDPOINT_DEPRECATED: { code: 'INTEG_ENDPOINT_DEPRECATED', display: 'Endpoint deprecated' },
    ...
    DIRECTION_INBOUND: { code: 'INTEG_DIRECTION_INBOUND', display: 'Inbound mapping' },
    DIRECTION_OUTBOUND: { code: 'INTEG_DIRECTION_OUTBOUND', display: 'Outbound mapping' },
    ...
    MSG_DEAD_LETTER: { code: 'INTEG_MSG_DEAD_LETTER', display: 'Outbound dead-lettered' },
    MSG_HELD: { code: 'INTEG_MSG_HELD', display: 'Outbound held (circuit open)' },
    MSG_ACKNOWLEDGED: { code: 'INTEG_MSG_ACKNOWLEDGED', display: 'Outbound acknowledged' },

    // --- Estado de reintento (message_retries.status_concept_id) ---
    RETRY_SCHEDULED: { code: 'INTEG_RETRY_SCHEDULED', display: 'Retry scheduled' },
    RETRY_FAILED: { code: 'INTEG_RETRY_FAILED', display: 'Retry failed' },
    RETRY_EXHAUSTED: { code: 'INTEG_RETRY_EXHAUSTED', display: 'Retries exhausted' },

    // --- Estado de mensaje entrante (inbound_messages.status_concept_id) ---
    INBOUND_RECEIVED: { code: 'INTEG_INBOUND_RECEIVED', display: 'Inbound received' },
    INBOUND_PROCESSED: { code: 'INTEG_INBOUND_PROCESSED', display: 'Inbound processed' },

    // --- Estado de suscripción de webhook (webhook_subscriptions.state_concept_id) ---
    WEBHOOK_ACTIVE: { code: 'INTEG_WEBHOOK_ACTIVE', display: 'Webhook subscription active' },
    WEBHOOK_DISABLED: { code: 'INTEG_WEBHOOK_DISABLED', display: 'Webhook subscription disabled' },
  },
);
