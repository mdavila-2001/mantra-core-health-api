...
function build() {
  ...
  const providersRepo = { findByCode: mockFn(), findById: mockFn(), create: mockFn() };
  const endpointsRepo = {
...
describe('IntegrationsProvidersService', () => {
  describe('registerProvider (UC-12-01)', () => {
    it('creates the provider ACTIVE and flushes', async () => {
      ...
      const created = { id: 'p1', code: 'LAB', name: 'Lab', stateConceptId: INTEG.PROVIDER_ACTIVE, createdAt: new Date() };
    ...
    it('rejects a duplicate code (conflict)', async () => {
      ...
      await expect(
        d.service.registerProvider({ code: 'LAB', name: 'Lab', providerType: 'LAB' } as any, actor),
  ...
  describe('publishEndpoint (UC-12-04)', () => {
    it('throws when the provider does not exist', async () => {
      ...
      await expect(
        d.service.publishEndpoint('p1', { code: 'c', operation: 'o', version: 'v1' } as any, actor),
      ).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('rejects when the provider is not active (precondition)', async () => {
      const d = build();
      d.providersRepo.findById.mockResolvedValue({ id: 'p1', stateConceptId: INTEG.PROVIDER_DRAFT });
      await expect(
        d.service.publishEndpoint('p1', { code: 'c', operation: 'o', version: 'v1' } as any, actor),
    ...
    it('publishes the endpoint, flushes the parent, then creates mappings', async () => {
      ...
      d.endpointsRepo.createEndpoint.mockReturnValue({ id: 'e1', code: 'c', version: 'v1', stateConceptId: INTEG.ENDPOINT_PUBLISHED });

      const res = await d.service.publishEndpoint(
        'p1',
        { code: 'c', operation: 'o', version: 'v1', mappings: [{ sourcePath: 'a', targetField: 'b', direction: 'OUTBOUND' }] } as any,
        actor,
      );
  ...
  describe('createWebhookSubscription (UC-12-11)', () => {
    it('creates a new subscription when none exists', async () => {
      ...
      d.webhooksRepo.create.mockReturnValue({ id: 'w1', eventType: 'e', stateConceptId: INTEG.WEBHOOK_ACTIVE });

      const res = await d.service.createWebhookSubscription(
        'p1',
        { eventType: 'e', callbackUrl: 'https://x.io/cb' } as any,
        actor,
      );
    ...
    it('updates an existing subscription (upsert)', async () => {
      const d = build();
      d.providersRepo.findById.mockResolvedValue({ id: 'p1', stateConceptId: INTEG.PROVIDER_ACTIVE });
      const existing = { id: 'w1', eventType: 'e', callbackUrl: 'old', stateConceptId: INTEG.WEBHOOK_DISABLED, updatedAt: new Date() };
      d.webhooksRepo.findByLogicalKey.mockResolvedValue(existing);

      const res = await d.service.createWebhookSubscription(
        'p1',
        { eventType: 'e', callbackUrl: 'https://x.io/new' } as any,
        actor,
      );
