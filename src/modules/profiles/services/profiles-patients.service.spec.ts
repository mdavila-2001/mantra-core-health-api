...
function build() {
  ...
  const personProfilesRepo = { findById: mockFn(), findByPersonAndType: mockFn(), create: mockFn() };
  const patientProfilesRepo = { findById: mockFn(), findByPatientCode: mockFn(), create: mockFn() };
  ...
  const mergeEventsRepo = { findById: mockFn(), findByReversalOf: mockFn(), create: mockFn() };
  ...
  const service = new ProfilesPatientsService(
    ...
    identityLinksRepo as any,
    mergeEventsRepo as any,
    relatedPersonsRepo as any,
    portalProxiesRepo as any,
    logger as any,
  );
  return {
    service, tx, personsRepo, personProfilesRepo, patientProfilesRepo, accountLinksRepo,
    identityLinksRepo, mergeEventsRepo, relatedPersonsRepo, portalProxiesRepo,
  };
}
...
describe('ProfilesPatientsService', () => {
  describe('registerPatient (UC-05-01)', () => {
    it('creates person, profile and patient flushing parent before child', async () => {
      ...
      d.personsRepo.create.mockReturnValue({ id: 'p1', createdAt: new Date('2026-01-01') });
      ...
      const res = await d.service.registerPatient({ patientCode: 'PC-1', displayName: 'Ada' } as any, actor);

      expect(res).toMatchObject({ profileId: 'pp1', personId: 'p1', patientCode: 'PC-1' });
      expect(d.tx.flush).toHaveBeenCalledTimes(3);
      expect(d.personProfilesRepo.create).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ profileTypeConceptId: PROF.PROFILE_TYPE_PATIENT }),
      );
    });

    it('rejects a duplicate patient_code (conflict)', async () => {
      const d = build();
      d.patientProfilesRepo.findByPatientCode.mockResolvedValue({ profileId: 'x' });
      await expect(
  ...
  describe('linkAccount (UC-05-02)', () => {
    ...
    it('supersedes the previous active link and creates a new one', async () => {
      const d = build();
      d.personsRepo.findById.mockResolvedValue({ id: 'p1', personStatusConceptId: PROF.PERSON_ACTIVE });
      d.accountLinksRepo.create.mockReturnValue({
        id: 'l1', statusConceptId: PROF.ACCOUNT_LINK_ACTIVE, validFrom: new Date(),
      });
      const res = await d.service.linkAccount('p1', { userId: 'u1' } as any, actor);
      expect(res).toMatchObject({ id: 'l1', personId: 'p1', userId: 'u1' });
      expect(d.accountLinksRepo.supersedeActiveForUser).toHaveBeenCalledWith(d.tx, 'u1', expect.any(Date));
    });
  });
  ...
  describe('mergePatients (UC-05-08)', () => {
    it('rejects merging a patient with itself (precondition)', async () => {
      ...
      await expect(
        d.service.mergePatients(
          { survivingPatientProfileId: 'a', mergedPatientProfileId: 'a' } as any,
          actor,
        ),
    ...
    it('rejects when a profile is missing (not found)', async () => {
      const d = build();
      d.patientProfilesRepo.findById.mockResolvedValueOnce({ profileId: 'a' }).mockResolvedValueOnce(null);
      await expect(
        d.service.mergePatients(
          { survivingPatientProfileId: 'a', mergedPatientProfileId: 'b' } as any,
          actor,
        ),
      ).rejects.toBeInstanceOf(ResourceNotFoundException);
    });
    ...
    it('records the merge, marks loser merged and reassigns references', async () => {
      const d = build();
      const merged = { profileId: 'b', recordLinkageStatusConceptId: PROF.LINKAGE_LINKED, updatedAt: new Date() };
      d.patientProfilesRepo.findById
        .mockResolvedValueOnce({ profileId: 'a' })
        .mockResolvedValueOnce(merged);
      d.mergeEventsRepo.create.mockReturnValue({
        id: 'e1', survivingPatientProfileId: 'a', mergedPatientProfileId: 'b',
        decisionStatusConceptId: PROF.MERGE_APPROVED, recordedAt: new Date(),
      });
      d.personProfilesRepo.findById.mockResolvedValue({ personId: 'per-b' });
      d.personsRepo.findById.mockResolvedValue({ id: 'per-b', updatedAt: new Date() });
      ...
      expect(res).toMatchObject({ id: 'e1', decisionStatus: PROF.MERGE_APPROVED });
      expect(merged.recordLinkageStatusConceptId).toBe(PROF.LINKAGE_MERGED);
      expect(d.identityLinksRepo.reassignPatientProfile).toHaveBeenCalledWith(d.tx, 'b', 'a', expect.any(Date));
    });
    ...
    it('rejects re-merging an already merged patient (conflict)', async () => {
      ...
      d.patientProfilesRepo.findById
        .mockResolvedValueOnce({ profileId: 'a' })
        .mockResolvedValueOnce({ profileId: 'b', recordLinkageStatusConceptId: PROF.LINKAGE_MERGED });
      await expect(
        d.service.mergePatients(
          { survivingPatientProfileId: 'a', mergedPatientProfileId: 'b' } as any,
          actor,
  ...
  describe('reverseMerge (UC-05-09)', () => {
    it('rejects reversing a non-approved event (precondition)', async () => {
      const d = build();
      d.mergeEventsRepo.findById.mockResolvedValue({ id: 'e1', decisionStatusConceptId: PROF.MERGE_REVERSED });
      await expect(d.service.reverseMerge('e1', {} as any, actor)).rejects.toBeInstanceOf(
        PreconditionFailedException,
      );
    });
    ...
    it('rejects reversing twice (conflict)', async () => {
      ...
      d.mergeEventsRepo.findById.mockResolvedValue({
        id: 'e1', decisionStatusConceptId: PROF.MERGE_APPROVED, reasonConceptId: PROF.MERGE_REASON_DUPLICATE,
        survivingPatientProfileId: 'a', mergedPatientProfileId: 'b',
      });
      d.mergeEventsRepo.findByReversalOf.mockResolvedValue({ id: 'e2' });
      await expect(d.service.reverseMerge('e1', {} as any, actor)).rejects.toBeInstanceOf(ConflictException);
    });
  });
  ...
  describe('addRelatedPerson (UC-05-10)', () => {
    it('rejects a second active legal guardian (conflict)', async () => {
      ...
      await expect(
        d.service.addRelatedPerson('a', { isLegalGuardian: true } as any, actor),
      ).rejects.toBeInstanceOf(ConflictException);
    });
    ...
    it('creates a new related person when none is referenced', async () => {
      ...
      d.relatedPersonsRepo.create.mockReturnValue({
        id: 'r1', statusConceptId: PROF.RELATED_ACTIVE, createdAt: new Date(),
      });
      const res = await d.service.addRelatedPerson('a', { displayName: 'Mom' } as any, actor);
      expect(res).toMatchObject({ id: 'r1', patientProfileId: 'a', personId: 'per-1' });
    });
  });
    ...
    it('throws when the patient does not exist', async () => {
      ...
      await expect(
        d.service.grantPortalProxy(
          'missing',
          { proxyUserId: 'u1', scopeValueSetId: 's1', legalBasisRecordId: 'lb1' } as any,
          actor,
        ),
      ).rejects.toBeInstanceOf(ResourceNotFoundException);
    });
  ...
  describe('decease (UC-05-12)', () => {
    it('rejects deceasing an already deceased person (conflict)', async () => {
      const d = build();
      d.personsRepo.findById.mockResolvedValue({ id: 'p1', vitalStatusConceptId: PROF.VITAL_DECEASED });
      await expect(d.service.decease('p1', {} as any, actor)).rejects.toBeInstanceOf(ConflictException);
    });

    it('marks deceased and revokes account links and proxies', async () => {
      const d = build();
      const person = { id: 'p1', vitalStatusConceptId: PROF.VITAL_ALIVE, updatedAt: new Date() } as any;
      ...
      const res = await d.service.decease('p1', { anonymize: true } as any, actor);
