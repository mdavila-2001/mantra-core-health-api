...
function build() {
  ...
  const followsRepo = { findByFollowerTarget: mockFn(), findMutualBetween: mockFn().mockResolvedValue([]), create: mockFn() };
  ...
  return { service, tx, profilesRepo, postsRepo, commentsRepo, reactionsRepo, bookmarksRepo, followsRepo, blocksRepo };
}
...
describe('CommunitySocialService', () => {
  describe('createProfile', () => {
    it('creates the public profile and returns its id/slug', async () => {
      const d = build();
      d.profilesRepo.create.mockReturnValue({ id: 'p1', slug: 's', displayName: 'N', statusConceptId: CONCEPTS.STATE_ACTIVE });
      const res = await d.service.createProfile(
        { tenantId: 't', targetId: 'u', slug: 's', displayName: 'N' } as any,
        actor,
      );
      expect(res).toEqual({ id: 'p1', slug: 's', displayName: 'N', status: CONCEPTS.STATE_ACTIVE });
  ...
  describe('publishPost (UC-19-01)', () => {
    it('throws when the author profile does not exist', async () => {
      ...
      await expect(d.service.publishPost('missing', { bodyText: 'x' } as any, actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
    ...
    it('flushes the post before children and links hashtags', async () => {
      const d = build();
      d.profilesRepo.findById.mockResolvedValue({ id: 'p1', commentsDefaultEnabled: true });
      d.postsRepo.create.mockReturnValue({ id: 'post1', authorPublicProfileId: 'p1', publicationStatusConceptId: 'pub', publishedAt: new Date() });
      d.postsRepo.upsertHashtag.mockResolvedValue({ id: 'h1' });
      const res = await d.service.publishPost(
        'p1',
        { bodyText: 'hi', hashtags: ['salud'], media: [{ fileId: 'f1' }], mentions: [{ mentionedProfileId: 'm1' }] } as any,
        actor,
      );
      expect(res.id).toBe('post1');
      expect(res.hashtagCount).toBe(1);
      expect(res.mediaCount).toBe(1);
      expect(d.postsRepo.linkHashtag).toHaveBeenCalledWith(d.tx, 'h1', 'post1', expect.any(String), actor.id);
  ...
  describe('createComment (UC-19-02)', () => {
    it('computes thread depth and increments the parent reply count', async () => {
      ...
      const parent = { id: 'c0', rootCommentId: null, threadDepth: 0, replyCount: 2, updatedAt: new Date() };
      ...
      const res = await d.service.createComment(
        { authorProfileId: 'p1', commentableType: 'POST', commentableRefId: 'post1', parentCommentId: 'c0', bodyText: 'hi' } as any,
        actor,
      );
    ...
    it('throws when the author profile does not exist', async () => {
      ...
      d.profilesRepo.findById.mockResolvedValue(null);
      await expect(
        d.service.createComment({ authorProfileId: 'x', commentableType: 'POST', commentableRefId: 'r', bodyText: 'y' } as any, actor),
  ...
  describe('react (UC-19-03)', () => {
    it('updates the existing reaction type (created=false)', async () => {
      const d = build();
      d.reactionsRepo.findByActorTarget.mockResolvedValue({ id: 'r1', reactionTypeConceptId: 'old', updatedAt: new Date() });
      const res = await d.service.react({ actorProfileId: 'p1', reactableType: 'POST', reactableRefId: 'post1', reactionType: 'LOVE' } as any, actor);
    ...
    it('creates a new reaction when none exists (created=true)', async () => {
      ...
      const res = await d.service.react({ actorProfileId: 'p1', reactableType: 'POST', reactableRefId: 'post1', reactionType: 'LIKE' } as any, actor);
  ...
  describe('bookmark (UC-19-04)', () => {
    it('rejects duplicate bookmarks', async () => {
      ...
      await expect(
        d.service.bookmark({ profileId: 'p1', bookmarkableType: 'POST', bookmarkableRefId: 'post1' } as any, actor),
  ...
  describe('follow (UC-19-05)', () => {
    it('rejects self-follow', async () => {
      const d = build();
      await expect(
        d.service.follow({ followerProfileId: 'p1', followableType: 'PROFILE', followableRefId: 'p1' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });

    it('rejects an already active follow (conflict)', async () => {
      const d = build();
      d.followsRepo.findByFollowerTarget.mockResolvedValue({ id: 'f1', statusConceptId: CONCEPTS.STATE_ACTIVE });
      await expect(
        d.service.follow({ followerProfileId: 'p1', followableType: 'PROFILE', followableRefId: 'p2' } as any, actor),
      ).rejects.toBeInstanceOf(ConflictException);
    });
  });
  ...
  describe('block (UC-19-14)', () => {
    it('rejects blocking oneself', async () => {
      const d = build();
      await expect(
        d.service.block({ blockerProfileId: 'p1', blockedProfileId: 'p1' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });
    ...
    it('creates the block and prunes mutual follows', async () => {
      ...
      const follow = { id: 'f1', statusConceptId: CONCEPTS.STATE_ACTIVE, updatedAt: new Date() };
      d.followsRepo.findMutualBetween.mockResolvedValue([follow]);
      const res = await d.service.block({ blockerProfileId: 'p1', blockedProfileId: 'p2', reason: 'SPAM' } as any, actor);
