...
function build() {
  ...
  const service = new CommunityPollsService(em as any, pollsRepo as any, postsRepo as any, logger as any);
...
describe('CommunityPollsService', () => {
  describe('createPoll (bootstrap)', () => {
    it('throws when the post does not exist', async () => {
      ...
      await expect(
        d.service.createPoll('missing', { question: 'q', options: ['a', 'b'] } as any, actor),
    ...
    it('creates the poll with its options', async () => {
      ...
      d.pollsRepo.createOption.mockReturnValueOnce({ id: 'o1' }).mockReturnValueOnce({ id: 'o2' });
      const res = await d.service.createPoll('post1', { question: 'q', options: ['a', 'b'] } as any, actor);
  ...
  describe('vote (UC-19-12)', () => {
    it('throws when the poll does not exist', async () => {
      ...
      await expect(
        d.service.vote('missing', { pollOptionId: 'o1', voterProfileId: 'p1' } as any, actor),
      ).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('rejects voting on a closed poll', async () => {
      const d = build();
      d.pollsRepo.findPollById.mockResolvedValue({ id: 'poll1', statusConceptId: COMM.POLL_CLOSED });
      await expect(
        d.service.vote('poll1', { pollOptionId: 'o1', voterProfileId: 'p1' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });

    it('rejects a second vote in a single-choice poll', async () => {
      const d = build();
      d.pollsRepo.findPollById.mockResolvedValue({ id: 'poll1', statusConceptId: COMM.POLL_OPEN, allowsMultiple: false });
      d.pollsRepo.findOptionById.mockResolvedValue({ id: 'o1', pollId: 'poll1' });
    ...
    it('records the vote and bumps counters', async () => {
      const d = build();
      const poll = { id: 'poll1', statusConceptId: COMM.POLL_OPEN, allowsMultiple: true, totalVotes: '2', updatedAt: new Date() };
      const option = { id: 'o1', pollId: 'poll1', voteCount: '1', updatedAt: new Date() };
      ...
      const res = await d.service.vote('poll1', { pollOptionId: 'o1', voterProfileId: 'p1' } as any, actor);
