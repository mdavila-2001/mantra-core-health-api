...
function build() {
  const service = { createConversation: mockFn(), sendMessage: mockFn(), markRead: mockFn() };
  return { controller: new CommunityMessagingController(service as any), service };
}
...
describe('CommunityMessagingController', () => {
  it('delegates createConversation', async () => {
    ...
    await d.controller.createConversation(dto as any, actor);
  ...
  it('delegates sendMessage (UC-19-06)', async () => {
    ...
    await d.controller.sendMessage('conv1', dto as any, actor);
  ...
  it('delegates markRead (UC-19-07)', async () => {
    ...
    await d.controller.markRead('conv1', dto as any, actor);
