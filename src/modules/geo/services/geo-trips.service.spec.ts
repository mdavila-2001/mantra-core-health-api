...
function build() {
  ...
  const tripsRepo = { findById: mockFn(), findInProgressBySession: mockFn(), create: mockFn() };
  const sessionsRepo = { findById: mockFn() };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };

  const service = new GeoTripsService(em as any, tripsRepo as any, sessionsRepo as any, logger as any);
...
describe('GeoTripsService', () => {
  describe('start (UC-13-06)', () => {
    it('starts a trip on an open session', async () => {
      ...
      const created = { id: 't1', trackingSessionId: 'sess-1', statusConceptId: GEO.TRIP_IN_PROGRESS };
      d.tripsRepo.create.mockReturnValue(created);

      const res = await d.service.start({ trackingSessionId: 'sess-1' } as any, actor);
    ...
    it('rejects when a trip is already in progress (conflict)', async () => {
      ...
      await expect(d.service.start({ trackingSessionId: 'sess-1' } as any, actor)).rejects.toBeInstanceOf(
        ConflictException,
      );
    });
    ...
    it('throws when the session does not exist', async () => {
      ...
      await expect(d.service.start({ trackingSessionId: 'x' } as any, actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
  });
  ...
  describe('close (UC-13-07)', () => {
    it('closes an in-progress trip and computes duration', async () => {
      ...
      const trip = { id: 't1', trackingSessionId: 'sess-1', statusConceptId: GEO.TRIP_IN_PROGRESS, startedAt: started, updatedAt: new Date() };
      d.tripsRepo.findById.mockResolvedValue(trip);

      const res = await d.service.close('t1', { distanceM: 1200 } as any, actor);
    ...
    it('rejects closing a trip not in progress (precondition)', async () => {
      const d = build();
      d.tripsRepo.findById.mockResolvedValue({ id: 't1', statusConceptId: GEO.TRIP_COMPLETED });
      await expect(d.service.close('t1', {} as any, actor)).rejects.toBeInstanceOf(
        PreconditionFailedException,
      );
    });
    ...
    it('throws when the trip does not exist', async () => {
      ...
      await expect(d.service.close('missing', {} as any, actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
  });
