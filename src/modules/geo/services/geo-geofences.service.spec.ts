...
function build() {
  ...
  const geofencesRepo = { findById: mockFn(), findByTenantAndName: mockFn(), create: mockFn() };
...
const circleDto = { tenantId: 't1', name: 'Zona', shapeType: 'CIRCLE', radiusM: 500, centerLat: -12, centerLng: -77 };
...
describe('GeoGeofencesService', () => {
  describe('define (UC-13-04)', () => {
    it('creates a circular geofence', async () => {
      ...
      expect(res).toMatchObject({ id: 'g1', shapeType: GEO.SHAPE_CIRCLE, state: GEO.GEOFENCE_ACTIVE });
    ...
    it('rejects a circle without center/radius (precondition, 422)', async () => {
      const d = build();
      await expect(
        d.service.define({ tenantId: 't1', name: 'Z', shapeType: 'CIRCLE' } as any, actor),
    ...
    it('rejects a polygon without geometryJson (precondition, 422)', async () => {
      const d = build();
      await expect(
        d.service.define({ tenantId: 't1', name: 'Z', shapeType: 'POLYGON' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });
    ...
    it('rejects a duplicated name (conflict)', async () => {
      ...
      await expect(d.service.define(circleDto as any, actor)).rejects.toBeInstanceOf(ConflictException);
    });
  });
  ...
  describe('recordEvent (UC-13-05)', () => {
    it('records an enter event and flushes', async () => {
      const d = build();
      d.geofencesRepo.findById.mockResolvedValue({ id: 'g1', stateConceptId: GEO.GEOFENCE_ACTIVE });
    ...
    it('rejects a duplicated consecutive transition (conflict)', async () => {
      ...
      d.geofencesRepo.findById.mockResolvedValue({ id: 'g1', stateConceptId: GEO.GEOFENCE_ACTIVE });
      d.subjectsRepo.findById.mockResolvedValue({ id: 's1' });
      d.eventsRepo.findLast.mockResolvedValue({ eventTypeConceptId: GEO.EVENT_ENTER });
      await expect(
        d.service.recordEvent({ geofenceId: 'g1', trackedSubjectId: 's1', eventType: 'ENTER' } as any, actor),
    ...
    it('throws when the geofence does not exist', async () => {
      ...
      await expect(
        d.service.recordEvent({ geofenceId: 'x', trackedSubjectId: 's1', eventType: 'ENTER' } as any, actor),
