...
function build() {
  const pharmaciesService = { createPharmacy: mockFn(), verifyLicense: mockFn() };
  const sitesService = { createSite: mockFn() };
  const productsService = { publishProduct: mockFn(), retireProduct: mockFn() };
  const pricingService = { createPriceList: mockFn(), versionPrice: mockFn(), closePriceList: mockFn() };
  const integrationService = { createConnection: mockFn(), mapProduct: mockFn() };
  ...
  return { controller, pharmaciesService, sitesService, productsService, pricingService, integrationService, catalogService };
}

describe('PharmacyController', () => {
  it('delegates createPharmacy (UC-24-01)', async () => {
    const d = build();
    const dto = { tenantId: 't1', code: 'PH-1', legalName: 'X', license: { licenseNumber: 'L' } };
    d.pharmaciesService.createPharmacy.mockResolvedValue({ id: 'ph1' });
    await expect(d.controller.createPharmacy(dto as any, actor)).resolves.toEqual({ id: 'ph1' });
    expect(d.pharmaciesService.createPharmacy).toHaveBeenCalledWith(dto, actor);
  });

  it('delegates verifyLicense (UC-24-03)', async () => {
    const d = build();
    await d.controller.verifyLicense('ph1', 'lic1', { approve: true } as any, actor);
    expect(d.pharmaciesService.verifyLicense).toHaveBeenCalledWith('ph1', 'lic1', { approve: true }, actor);
  });

  it('delegates createSite (UC-24-02)', async () => {
    const d = build();
    const dto = { practiceSiteId: 'ps1', code: 'S', name: 'N' };
    await d.controller.createSite('ph1', dto as any, actor);
  ...
  it('delegates publishProduct (UC-24-04)', async () => {
    ...
    await d.controller.publishProduct('ph1', dto as any, actor);
    expect(d.productsService.publishProduct).toHaveBeenCalledWith('ph1', dto, actor);
  });

  it('delegates retireProduct (UC-24-09)', async () => {
    const d = build();
    await d.controller.retireProduct('ph1', 'pr1', actor);
    expect(d.productsService.retireProduct).toHaveBeenCalledWith('ph1', 'pr1', actor);
  });
  ...
  it('delegates createPriceList (UC-24-05)', async () => {
    ...
    expect(d.pricingService.createPriceList).toHaveBeenCalledWith('ph1', dto, actor);
  });

  it('delegates versionPrice (UC-24-06)', async () => {
    const d = build();
    const dto = { pharmacyProductId: 'pr1', unitAmount: 10 };
    await d.controller.versionPrice('ph1', 'pl1', dto as any, actor);
    expect(d.pricingService.versionPrice).toHaveBeenCalledWith('ph1', 'pl1', dto, actor);
  });

  it('delegates closePriceList (UC-24-10)', async () => {
    const d = build();
    await d.controller.closePriceList('ph1', 'pl1', actor);
    expect(d.pricingService.closePriceList).toHaveBeenCalledWith('ph1', 'pl1', actor);
  });
  ...
  it('delegates createConnection (UC-24-07)', async () => {
    ...
    expect(d.integrationService.createConnection).toHaveBeenCalledWith('ph1', dto, actor);
  });

  it('delegates mapProduct (UC-24-08)', async () => {
    const d = build();
    const dto = { pharmacyProductId: 'pr1', externalProductCode: 'X' };
    await d.controller.mapProduct('ph1', 'conn1', dto as any, actor);
    expect(d.integrationService.mapProduct).toHaveBeenCalledWith('ph1', 'conn1', dto, actor);
  });
