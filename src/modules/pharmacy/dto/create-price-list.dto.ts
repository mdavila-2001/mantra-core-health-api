...
export class CreatePriceListDto {
  @ApiProperty({ description: 'Código único de lista por farmacia', maxLength: 100 })
  @IsString()
  @MinLength(1)
  @MaxLength(100)
  code!: string;

  @ApiProperty({ description: 'Tipo de lista de precios', enum: ['PUBLIC', 'INSURER'] })
  @IsIn(['PUBLIC', 'INSURER'])
  priceListType!: PriceListTypeCode;

  @ApiPropertyOptional({ description: 'Sede a la que aplica la lista', format: 'uuid' })
  @IsOptional()
  @IsUUID()
  pharmacySiteId?: string;

  @ApiPropertyOptional({ description: 'Tenant aseguradora (obligatorio si tipo INSURER)', format: 'uuid' })
  @IsOptional()
  @IsUUID()
  insurerTenantId?: string;

  @ApiPropertyOptional({ description: 'Concept id de la moneda', format: 'uuid' })
  @IsOptional()
  @IsUUID()
