...
Capa HTTP fina del módulo 23. Validan parámetros (`ParseUUIDPipe`) y delegan en
el servicio de dominio. Todos requieren bearer token (guard global;
`@ApiBearerAuth`); las lecturas usan el tenant activo y los comandos aplican
`@Roles('SECURITY_ADMIN')`.

| Controlador | Rutas | UC |
|---|---|---|
| `DiagnosticUnitsController` | `GET /diagnostic-units`, `GET /diagnostic-units/{id}`, `POST /diagnostic-units`, `/{id}/sites`, `/{id}/verify-and-publish`, `PUT /{id}/specialties`, `POST /{id}/study-offerings`, `/{id}/price-schedules`, `/{id}/practitioner-assignments`, `/{id}/accreditations`, `/{id}/reproject` | Directorio P3, 01,02,03,04,05,06,10,11,12 |
