...
export class UserGlobalRolesRepository {
  ...
  create(em: EntityManager, data: CreateRoleData): UserGlobalRoles {
    return em.create(UserGlobalRoles, {
      userId: data.userId,
  ...
  findActiveForUser(em: EntityManager, userId: string): Promise<UserGlobalRoles[]> {
    return em.find(UserGlobalRoles, { userId, stateConceptId: CONCEPTS.STATE_ACTIVE });
  }
