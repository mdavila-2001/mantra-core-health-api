...
import { SettlementRepository, ClaimRepository, CatalogRepository } from '../repositories';
...
export class ReconciliationService {
  ...
  async createBatch(dto: CreateReconciliationBatchDto, actor: AuthenticatedUser): Promise<ResourceStatusDto> {
    return this.em.transactional(async (tx) => {
      ...
      await tx.flush();
      this.logger.info({ operation: 'insurance.recon.batch', batchId: batch.id }, 'Reconciliation batch opened');
      return { id: batch.id, status: batch.statusConceptId, createdAt: batch.createdAt };
    });
  }

  /** UC-26-13b: agregar ítem al lote conciliando contra la versión de adjudicación. */
  async addItem(batchId: string, dto: CreateReconciliationItemDto, actor: AuthenticatedUser): Promise<CreatedResourceDto> {
    return this.em.transactional(async (tx) => {
      const batch = await this.repo.findBatch(tx, batchId);
      if (!batch) throw new ResourceNotFoundException('Lote de conciliación no encontrado', { batchId });
      const claim = await this.claims.findClaim(tx, dto.insuranceClaimId);
      if (!claim) throw new ResourceNotFoundException('Reclamo no encontrado', { claimId: dto.insuranceClaimId });
      const version = await this.claims.findVersion(tx, dto.claimAdjudicationVersionId);
      ...
      const expected = dto.expectedAmount !== undefined ? Number(dto.expectedAmount) : 0;
      const accepted = dto.acceptedAmount !== undefined ? Number(dto.acceptedAmount) : 0;
      ...
      const item = this.repo.createItem(tx, {
        ...
        varianceReasonConceptId: variance === '0.00' ? INS.VARIANCE_NONE : undefined,
