...
function build() {
  ...
  const repo = { createPolicy: mockFn(), findPolicyById: mockFn(), createTestRun: mockFn() };
...
describe('BackupService', () => {
  describe('createPolicy (UC-11-09)', () => {
    it('rejects when rpo > rto', async () => {
      const d = build();
      await expect(
        d.service.createPolicy({ rpoSeconds: 100, rtoSeconds: 10 } as any, actor),
    ...
    it('creates the policy', async () => {
      ...
      const res = await d.service.createPolicy({ rpoSeconds: 10, rtoSeconds: 100 } as any, actor);
  ...
  describe('recordRestoreTest (UC-11-10)', () => {
    it('throws when the policy is missing', async () => {
      ...
      await expect(
        d.service.recordRestoreTest({ backupPolicyId: 'bp1', outcomeConceptId: 'o' } as any, actor),
    ...
    it('flags an objective breach when measured RPO exceeds target', async () => {
      ...
      d.repo.createTestRun.mockReturnValue({ id: 'rt1', outcomeConceptId: 'o' });
      const res = await d.service.recordRestoreTest(
        { backupPolicyId: 'bp1', outcomeConceptId: 'o', measuredRpoSeconds: 120 } as any,
        actor,
      );
