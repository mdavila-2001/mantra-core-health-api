...
function build() {
  const invoicesService = { issueFromEncounter: mockFn(), creditNote: mockFn(), createPaymentPlan: mockFn() };
  ...
  return { controller, invoicesService, paymentsReceivedService, reimbursementsService, statementsService };
}
...
describe('BillingReceivablesController', () => {
  it('delegates issueFromEncounter (UC-17-01)', async () => {
    ...
    await expect(d.controller.issueFromEncounter(dto as any, actor)).resolves.toEqual({ id: 'inv1' });
    expect(d.invoicesService.issueFromEncounter).toHaveBeenCalledWith(dto, actor);
  });
  ...
  it('delegates creditNote (UC-17-03)', async () => {
    ...
    expect(d.invoicesService.creditNote).toHaveBeenCalledWith('inv1', dto, actor);
  });

  it('delegates linkReimbursement (UC-17-08)', async () => {
    const d = build();
    const dto = { claimId: 'c1', invoiceId: 'inv1', amount: '1.00' };
    await d.controller.linkReimbursement(dto as any, actor);
  ...
  it('delegates createPaymentPlan (UC-17-11)', async () => {
    ...
    expect(d.invoicesService.createPaymentPlan).toHaveBeenCalledWith(dto, actor);
  });
});
