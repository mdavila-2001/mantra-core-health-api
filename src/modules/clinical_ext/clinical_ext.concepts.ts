 ...
 */
export const { seeds: CLINICAL_EXT_CONCEPT_SEEDS, ids: CEXT } = defineModuleConcepts(
  'clinical_ext',
  {
    ...
    MEMBER_ROLE_ATTENDER: { code: 'CTM_ATTENDER', display: 'Attending member role' },
    ...
    CDS_RULE_TYPE_ALERT: { code: 'CDS_TYPE_ALERT', display: 'Alert-type CDS rule' },
    ...
    ALERT_ACKNOWLEDGED: { code: 'ALRT_ACK', display: 'Clinical alert acknowledged' },
    ALERT_OVERRIDDEN: { code: 'ALRT_OVERRIDDEN', display: 'Clinical alert overridden' },
    ALERT_TYPE_CDS: { code: 'ALRT_CDS', display: 'CDS alert' },
    ALERT_TYPE_DRUG_INTERACTION: {
      code: 'ALRT_DDI',
      display: 'Drug-drug interaction alert',
    },
    TRIGGER_CLINICAL_EVENT: { code: 'TRG_CLINICAL', display: 'Clinical event trigger' },

    // --- order_sets ----------------------------------------------------------
    ORDER_SET_ACTIVE: { code: 'OS_ACTIVE', display: 'Order set active' },
    ORDER_ITEM_TYPE_MEDICATION: { code: 'OSI_MEDICATION', display: 'Medication order item' },
    ORDER_ITEM_TYPE_SERVICE: { code: 'OSI_SERVICE', display: 'Service order item' },

    // --- referrals -----------------------------------------------------------
    REFERRAL_REQUESTED: { code: 'REF_REQUESTED', display: 'Referral requested' },
    ...
    REFERRAL_PRIORITY_ROUTINE: { code: 'REF_ROUTINE', display: 'Routine referral priority' },

    // --- care_gaps -----------------------------------------------------------
    CARE_GAP_OPEN: { code: 'GAP_OPEN', display: 'Care gap open' },
    CARE_GAP_CLOSED: { code: 'GAP_CLOSED', display: 'Care gap closed' },
    GAP_TYPE_IMMUNIZATION: { code: 'GAP_IMMUNIZATION', display: 'Immunization gap' },
    ...
    // --- virtual_encounters --------------------------------------------------
    VIRTUAL_ENCOUNTER_SCHEDULED: { code: 'VENC_SCHEDULED', display: 'Virtual encounter scheduled' },
    VIRTUAL_ENCOUNTER_IN_PROGRESS: {
      code: 'VENC_IN_PROGRESS',
      display: 'Virtual encounter in progress',
    },
    VIRTUAL_ENCOUNTER_COMPLETED: { code: 'VENC_COMPLETED', display: 'Virtual encounter completed' },
  },
);
