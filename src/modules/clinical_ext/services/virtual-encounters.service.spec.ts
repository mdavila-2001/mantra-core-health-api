...
function build() {
  ...
  const encountersRepo = { findById: mockFn(), findByEncounter: mockFn(), create: mockFn() };
  const logger = { setContext: mockFn(), info: mockFn() };
  const service = new VirtualEncountersService(em as any, encountersRepo as any, logger as any);
...
describe('VirtualEncountersService (UC-18-12)', () => {
  it('creates a scheduled session', async () => {
    ...
    d.encountersRepo.create.mockReturnValue({ id: 've1', encounterId: 'e1', statusConceptId: CEXT.VIRTUAL_ENCOUNTER_SCHEDULED });
    const res = await d.service.create({ encounterId: 'e1' } as any, actor);
  ...
  it('rejects a second session for the same encounter (conflict)', async () => {
    ...
    await expect(d.service.create({ encounterId: 'e1' } as any, actor)).rejects.toBeInstanceOf(ConflictException);
  });

  it('joins a scheduled session (scheduled -> in-progress)', async () => {
    const d = build();
    const venc = { id: 've1', encounterId: 'e1', statusConceptId: CEXT.VIRTUAL_ENCOUNTER_SCHEDULED, updatedAt: new Date() };
  ...
  it('rejects joining a non-scheduled session (precondition)', async () => {
    const d = build();
    d.encountersRepo.findById.mockResolvedValue({ id: 've1', statusConceptId: CEXT.VIRTUAL_ENCOUNTER_COMPLETED });
    await expect(d.service.join('ve1', actor)).rejects.toBeInstanceOf(PreconditionFailedException);
  });

  it('throws when ending a missing session', async () => {
    const d = build();
    d.encountersRepo.findById.mockResolvedValue(null);
    await expect(d.service.end('ve1', {} as any, actor)).rejects.toBeInstanceOf(ResourceNotFoundException);
  });

  it('ends an in-progress session (in-progress -> completed)', async () => {
    const d = build();
    const venc: any = { id: 've1', encounterId: 'e1', statusConceptId: CEXT.VIRTUAL_ENCOUNTER_IN_PROGRESS, updatedAt: new Date() };
    d.encountersRepo.findById.mockResolvedValue(venc);
    const res = await d.service.end('ve1', { recordingFileId: 'f1' } as any, actor);
