...
function build() {
  ...
  const service = new CareGapsService(em as any, gapsRepo as any, schedulesRepo as any, logger as any);
...
describe('CareGapsService', () => {
  describe('recompute (UC-18-09)', () => {
    it('opens new gaps and skips already-open ones', async () => {
      const d = build();
      d.gapsRepo.findOpen.mockImplementation((_tx: any, _p: string, gapType: string) =>
        gapType === 'existing' ? { id: 'g0' } : null,
      );
      d.gapsRepo.create.mockImplementation((_tx: any, data: any) => ({ id: 'g1', ...data }));

      const res = await d.service.recompute(
        { patientProfileId: 'p1', gaps: [{ gapTypeConceptId: 'new' }, { gapTypeConceptId: 'existing' }] } as any,
        actor,
      );
  ...
  describe('close (UC-18-10)', () => {
    it('closes an open gap', async () => {
      const d = build();
      const gap = { id: 'g1', statusConceptId: CEXT.CARE_GAP_OPEN, updatedAt: new Date() };
      d.gapsRepo.findById.mockResolvedValue(gap);
      const res = await d.service.close('g1', {} as any, actor);
    ...
    it('throws when the gap is missing', async () => {
      ...
      await expect(d.service.close('g1', {} as any, actor)).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('rejects closing a non-open gap (precondition)', async () => {
      const d = build();
      d.gapsRepo.findById.mockResolvedValue({ id: 'g1', statusConceptId: CEXT.CARE_GAP_CLOSED });
      await expect(d.service.close('g1', {} as any, actor)).rejects.toBeInstanceOf(PreconditionFailedException);
    });
  });
  ...
  describe('projectImmunizationPlan (UC-18-11)', () => {
    it('opens a gap per pending dose using the recommended age', async () => {
      ...
      d.gapsRepo.create.mockImplementation((_tx: any, data: any) => ({ id: `g-${data.measureConceptId}`, ...data }));

      const res = await d.service.projectImmunizationPlan('p1', { birthDate: '2026-01-01' } as any, actor);
    ...
    it('skips doses that already have an open gap', async () => {
      const d = build();
      d.schedulesRepo.findActive.mockResolvedValue([{ id: 's1', vaccineConceptId: 'v1', recommendedAgeDays: 60 }]);
