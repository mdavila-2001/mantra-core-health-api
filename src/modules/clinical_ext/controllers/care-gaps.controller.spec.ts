...
describe('CareGapsController', () => {
  it('delegates recompute (UC-18-09)', async () => {
    ...
    await d.controller.recompute(dto as any, actor);
  ...
  it('delegates close (UC-18-10)', async () => {
    ...
    await d.controller.close('g1', dto as any, actor);
  ...
  it('delegates projectImmunizationPlan (UC-18-11)', async () => {
    ...
    await d.controller.projectImmunizationPlan('p1', dto as any, actor);
    expect(d.careGapsService.projectImmunizationPlan).toHaveBeenCalledWith('p1', dto, actor);
  });

  it('delegates createSchedule', async () => {
    const d = build();
    const dto = { vaccineConceptId: 'v1', name: 'BCG' };
    await d.controller.createSchedule(dto as any, actor);
