...
function build() {
  ...
  const controller = new ClinicalObservationsController(observationsService as any);
...
describe('ClinicalObservationsController', () => {
  it('delegates record (UC-08-03)', async () => {
    ...
    await expect(d.controller.record(dto as any, actor)).resolves.toEqual({ id: 'obs1' });
  ...
  it('delegates amend (UC-08-04)', async () => {
    const d = build();
    const dto = { note: 'fix' };
    await d.controller.amend('obs1', dto as any, actor);
    expect(d.observationsService.amend).toHaveBeenCalledWith('obs1', dto, actor);
  });
});
