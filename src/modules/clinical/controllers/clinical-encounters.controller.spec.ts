...
describe('ClinicalEncountersController', () => {
  it('delegates openEpisode (UC-08-01)', async () => {
    ...
    await expect(d.controller.openEpisode(dto as any, actor)).resolves.toEqual({
      id: 'ep1',
    });
  ...
  it('delegates checkIn (UC-08-02)', async () => {
    const d = build();
    const dto = { patientProfileId: 'p1', tenantId: 't1' };
    await d.controller.checkIn(dto, actor);
    expect(d.encountersService.checkIn).toHaveBeenCalledWith(dto, actor);
  });

  it('delegates close (UC-08-14)', async () => {
    const d = build();
    await d.controller.close('enc1', { expectedRowVersion: 1 }, actor);
    expect(d.encountersService.close).toHaveBeenCalledWith(
      'enc1',
