...
/** Query de `GET /scheduling/bookings`. */
export class ListBookingsQueryDto {
  ...
  @ApiPropertyOptional({
    format: 'uuid',
    description: 'Citas de un paciente concreto',
  })
  @IsOptional()
  @IsUUID()
  ...
  @ApiPropertyOptional({
    format: 'uuid',
    description: 'Citas de un recurso concreto (la agenda del profesional)',
  })
  @IsOptional()
  @IsUUID()
  resourceId?: string;
  ...
  @ApiPropertyOptional({
    format: 'date-time',
    description: 'Inicio de la ventana, sobre el instante del cupo',
  })
  @IsOptional()
  @IsISO8601()
  from?: string;
...
/**
 * Una cita con el instante ya resuelto.
 *
 * `startAt`/`endAt` vienen del cupo, no de la cita: la tabla de citas no guarda
 * el instante, y sin él una lista de citas no se puede ni ordenar ni pintar.
 */
export class BookingListItemDto {
  ...
  /**
   * Instante de inicio, tomado del cupo.
   */
  @ApiPropertyOptional({ format: 'date-time', nullable: true })
  startAt!: string | null;

  /**
   * Instante de fin, tomado del cupo.
   */
  @ApiPropertyOptional({ format: 'date-time', nullable: true })
  endAt!: string | null;
...
/** Respuesta de `GET /scheduling/bookings`. */
export class ListBookingsResponseDto {
  /**
   * Citas de esta página, de la más próxima a la más lejana.
   */
  ...
  /**
   * Si el filtro devolvía más citas de las que entran en el tope.
   */
  @ApiProperty()
  truncated!: boolean;
}

/** Respuesta de `GET /scheduling/bookings/{id}`. */
export class BookingDetailDto extends BookingListItemDto {
  ...
  /**
   * Condiciones de cancelación congeladas al confirmar (CAN-APT-001). Es lo que
   * el front debe mostrar al ofrecer cancelar: la política vigente puede haber
   * cambiado desde que el paciente aceptó.
   */
  @ApiPropertyOptional({
    nullable: true,
    description: 'Snapshot de la política de cancelación aceptada',
  })
...
/** Un recordatorio programado de la cita. */
