...
function build() {
  ...
  const consentsRepo = { findById: mockFn(), findLatest: mockFn(), create: mockFn() };
  const purposesRepo = { findById: mockFn() };
  const subjectsRepo = { findByKey: mockFn(), create: mockFn(), findActiveByConsent: mockFn().mockResolvedValue([]) };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };

  const service = new TelemetryConsentService(
    em as any,
    disclosuresRepo as any,
    acceptancesRepo as any,
    consentsRepo as any,
    purposesRepo as any,
    subjectsRepo as any,
    logger as any,
  );
  return { service, tx, disclosuresRepo, acceptancesRepo, consentsRepo, purposesRepo, subjectsRepo };
}
...
describe('TelemetryConsentService', () => {
  describe('acceptDisclosure (UC-28-04)', () => {
    it('records a new acceptance', async () => {
      ...
      d.acceptancesRepo.create.mockReturnValue({ id: 'a1', trackingDisclosureVersionId: 'v1', userId: 'user-1' });
      const res = await d.service.acceptDisclosure({ trackingDisclosureVersionId: 'v1' } as any, actor);
    ...
    it('is idempotent when an acceptance already exists', async () => {
      ...
      d.acceptancesRepo.findExisting.mockResolvedValue({
        id: 'a0', trackingDisclosureVersionId: 'v1', userId: 'user-1', createdAt: new Date(),
      });
      const res = await d.service.acceptDisclosure({ trackingDisclosureVersionId: 'v1' } as any, actor);
    ...
    it('404 when the disclosure version is missing', async () => {
      ...
      await expect(
        d.service.acceptDisclosure({ trackingDisclosureVersionId: 'v9' } as any, actor),
  ...
  describe('grantConsent (UC-28-05)', () => {
    it('grants consent for a purpose that requires it', async () => {
      const d = build();
      d.purposesRepo.findById.mockResolvedValue({ id: 'p1', requiresConsent: true });
      d.consentsRepo.create.mockReturnValue({
        id: 'c1', userId: 'user-1', purposeDefinitionId: 'p1', decisionConceptId: TELE.DECISION_GRANTED,
      });
      const res = await d.service.grantConsent({ purposeDefinitionId: 'p1' } as any, actor);
      expect(res.decisionConceptId).toBe(TELE.DECISION_GRANTED);
    });

    it('422 when the purpose does not require consent', async () => {
      const d = build();
      d.purposesRepo.findById.mockResolvedValue({ id: 'p1', requiresConsent: false });
      await expect(
  ...
  describe('provisionSubject (UC-28-06)', () => {
    it('reuses an existing subject by key (idempotent)', async () => {
      const d = build();
      d.subjectsRepo.findByKey.mockResolvedValue({ id: 's0', pseudonymousSubjectKey: 'k', createdAt: new Date() });
      const res = await d.service.provisionSubject({ pseudonymousSubjectKey: 'k' } as any);
    ...
    it('creates a new subject when the key is free', async () => {
      ...
      d.subjectsRepo.create.mockReturnValue({ id: 's1', pseudonymousSubjectKey: 'k', createdAt: new Date() });
      const res = await d.service.provisionSubject({ pseudonymousSubjectKey: 'k' } as any);
  ...
  describe('withdrawConsent (UC-28-12)', () => {
    it('withdraws and deactivates subjects in cascade', async () => {
      const d = build();
      d.consentsRepo.findById.mockResolvedValue({ id: 'c1', userId: 'user-1', purposeDefinitionId: 'p1' });
      d.consentsRepo.findLatest.mockResolvedValue({ id: 'c1', decisionConceptId: TELE.DECISION_GRANTED });
      d.consentsRepo.create.mockReturnValue({
        id: 'c2', userId: 'user-1', purposeDefinitionId: 'p1', decisionConceptId: TELE.DECISION_WITHDRAWN,
      });
    ...
    it('404 when the consent does not exist', async () => {
      ...
      await expect(d.service.withdrawConsent('c9', actor)).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('422 when there is no active granted consent', async () => {
      const d = build();
      d.consentsRepo.findById.mockResolvedValue({ id: 'c1', userId: 'user-1', purposeDefinitionId: 'p1' });
      d.consentsRepo.findLatest.mockResolvedValue({ id: 'c1', decisionConceptId: TELE.DECISION_WITHDRAWN });
      await expect(d.service.withdrawConsent('c1', actor)).rejects.toBeInstanceOf(PreconditionFailedException);
    });
  });
});
