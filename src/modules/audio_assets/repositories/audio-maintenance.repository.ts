...
export class AudioMaintenanceRepository {
  ...
  listReady(limit: number): Promise<AudioAssets[]> {
    return this.em.find(AudioAssets, { generationStatus: 'READY' }, { orderBy: { generatedAt: 'ASC' }, limit });
  }
  async listGarbageCandidates(cutoff: Date, limit: number): Promise<AudioAssets[]> {
    ...
    return this.em.find(AudioAssets, { id: { $in: rows.map((row) => row.id) } });
  }
  async deprecate(assetId: string): Promise<boolean> {
    ...
    asset.generationStatus = 'DEPRECATED'; asset.updatedAt = new Date(); await this.em.flush(); return true;
  }
  async storageReferenceCount(storageKey: string, excludingId: string): Promise<number> {
  ...
  async clearStorageReference(assetId: string): Promise<void> {
    await this.em.getConnection().execute(
      `update audio_assets.audio_assets set storage_key=null, storage_provider=null, updated_at=now() where id=?`, [assetId], 'run');
  }
  async statusCounts(): Promise<Record<string, number>> {
    const rows = await this.em.getConnection().execute<Array<{ generation_status: string; count: number }>>(
      `select generation_status, count(*)::int as count from audio_assets.audio_assets group by generation_status`, [], 'all');
    return Object.fromEntries(rows.map((row) => [row.generation_status, row.count]));
  }
}
