...
describe('ResolveAudioAssetUseCase concurrency', () => {
  ...
  it('tres solicitudes iguales convergen al mismo asset/dedupe key', async () => {
    ...
    const repository: any = {
      ...
      dynamicFields: jest.fn(() => []),
      findAssetByKey: jest.fn(async (key: string) => assets.get(key) ?? null),
      findReusableReady: jest.fn(async () => null),
      createPendingOrGet: jest.fn(async (input: any) => {
        if (!assets.has(input.assetKey))
    ...
    const budget: any = {
      canGenerate: jest.fn(async () => ({ allowed: true })),
    };
    const cipher: any = { encrypt: jest.fn((text: string) => `enc:${text}`) };
    const queue: any = {
      enqueue: jest.fn(async (_id: string, key: string) => {
        dedupe.add(key);
        return 'job-1';
      }),
    };
    ...
    const tracing: any = {
      runInSpan: jest.fn(
        async (_n: string, _a: unknown, op: () => Promise<unknown>) => op(),
      ),
      addEvent: jest.fn(),
    };
    ...
    const results = await Promise.all(
      Array.from({ length: 3 }, () =>
        useCase.execute({ templateKey: 'onboarding.dashboard.intro' }, actor),
      ),
    );
    expect(new Set(results.map((result) => result.asset?.id))).toEqual(
      new Set(['asset-1']),
    );
