...
export const audioEnvSchema = Joi.object({
  ...
  AUDIO_TTS_MAX_REQUESTS_PER_SECOND: Joi.number()
    ...
    .max(100)
    .default(2),
  AUDIO_TTS_GC_RETENTION_DAYS: Joi.number()
    .integer()
    .min(1)
    .max(3650)
    .default(90),
  AUDIO_TTS_DATA_KEY: Joi.string()
    .allow('')
    .default(() =>
      process.env.NODE_ENV === 'production'
        ? ''
        : 'dev-only-audio-data-key-change-before-production',
    ),
...
export function loadAudioEnv(): AudioEnv {
  const provider = (process.env.AUDIO_TTS_PROVIDER ??
    'disabled') as AudioTtsProviderName;
  const dataKey =
  ...
  if (process.env.NODE_ENV === 'production' && dataKey.length < 32) {
    throw new Error(
      'AUDIO_TTS_DATA_KEY (>=32 caracteres) es obligatoria en producción',
    );
  }
  ...
  const model =
    provider === 'elevenlabs' && process.env.ELEVENLABS_MODEL_ID
      ...
      : (process.env.AUDIO_TTS_MODEL ??
        (provider === 'fake' ? 'fake-v1' : 'eleven_v3'));
  const format =
  ...
  return {
    ...
    monthlyBudgetUnits: Number(
      process.env.AUDIO_TTS_MONTHLY_BUDGET_UNITS ?? 10000,
    ),
    safetyReserveUnits: Number(
      process.env.AUDIO_TTS_SAFETY_RESERVE_UNITS ?? 1000,
    ),
    runtimeGenerationsPerActorDay: Number(
      process.env.AUDIO_TTS_RUNTIME_GENERATIONS_PER_ACTOR_DAY ?? 0,
    ),
    ...
    prodLicenseConfirmed:
      process.env.AUDIO_TTS_PROD_LICENSE_CONFIRMED === 'true',
    globalFallbackTemplate:
      process.env.AUDIO_TTS_GLOBAL_FALLBACK_TEMPLATE ??
      'onboarding.fallback.generic',
    crossTemplateDedup: process.env.AUDIO_TTS_CROSS_TEMPLATE_DEDUP !== 'false',
    circuitFailureThreshold: Number(
      process.env.AUDIO_TTS_CB_FAILURE_THRESHOLD ?? 5,
    ),
    circuitOpenMs: Number(process.env.AUDIO_TTS_CB_OPEN_MS ?? 30000),
    maxConcurrency: Number(process.env.AUDIO_TTS_MAX_CONCURRENCY ?? 2),
    maxRequestsPerSecond: Number(
      process.env.AUDIO_TTS_MAX_REQUESTS_PER_SECOND ?? 2,
    ),
    gcRetentionDays: Number(process.env.AUDIO_TTS_GC_RETENTION_DAYS ?? 90),
    dataKey,
