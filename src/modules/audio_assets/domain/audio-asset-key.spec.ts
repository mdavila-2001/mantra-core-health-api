...
describe('audio asset keys', () => {
  ...
  it('mantiene asset_key específico por plantilla', () => {
    ...
    expect(buildAudioAssetKey({ ...base, templateId: 'a' })).not.toBe(
      buildAudioAssetKey({ ...base, templateId: 'b' }),
    );
  });

  it('permite reutilizar síntesis idéntica entre plantillas', () => {
    const a = buildAudioSynthesisFingerprint({
      ...profile,
      normalizedText: 'hola pablo',
    });
    const b = buildAudioSynthesisFingerprint({
      ...profile,
      normalizedText: 'hola pablo',
    });
    expect(a).toBe(b);
  ...
  it('invalida la identidad cuando cambia la voz o el modelo', () => {
    const base = buildAudioSynthesisFingerprint({
      ...profile,
      normalizedText: 'hola',
    });
    expect(
