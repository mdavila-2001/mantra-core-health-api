...
export function normalizeDynamicValue(
  ...
  if (rawValue === undefined || rawValue.trim() === '') {
    if (field.required) throw new InvalidDynamicAudioValueError(field.name, 'requerido');
    return undefined;
  }
  const compact = rawValue.normalize('NFKC').trim().replace(/\s+/gu, ' ');
  const maxLength = field.maxLength ?? (field.type === 'PERSON_NAME' ? 60 : 120);
  if (compact.length > maxLength) {
    throw new InvalidDynamicAudioValueError(field.name, 'longitud excedida');
  }
  if (
    CONTROL_CHARS.test(compact) ||
    HTML_LIKE.test(compact) ||
    URL_LIKE.test(compact) ||
    EMAIL_LIKE.test(compact) ||
    TOKEN_LIKE.test(compact)
  ) {
    throw new InvalidDynamicAudioValueError(field.name, 'contenido no permitido');
  }
  ...
  switch (field.type) {
    ...
    case 'SAFE_TEXT':
      if (!SAFE_TEXT.test(value)) {
        throw new InvalidDynamicAudioValueError(field.name, 'texto no permitido');
      }
      return value;
    case 'ENUM': {
      const allowed = field.allowedValues ?? [];
      const match = allowed.find((item) => item.toLocaleLowerCase('es') === value.toLocaleLowerCase('es'));
      if (!match) throw new InvalidDynamicAudioValueError(field.name, 'fuera del catálogo');
      return match;
    }
  }
}
...
function titleCaseName(value: string): string {
  return value
    ...
    .split(' ')
    .map((part) => part.replace(/^\p{L}/u, (letter) => letter.toLocaleUpperCase('es')))
    .join(' ');
}
