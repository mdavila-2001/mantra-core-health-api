...
function build() {
  const rolesService = {
    createRole: mockFn(),
    setPermissions: mockFn(),
    setFieldPermissions: mockFn(),
  };
...
describe('AuthzRolesController', () => {
  it('delegates createRole (UC-06-03)', async () => {
    ...
    await d.controller.createRole(dto, actor);
  ...
  it('delegates setPermissions (UC-06-03)', async () => {
    ...
    await d.controller.setPermissions('r1', dto, actor);
    expect(d.rolesService.setPermissions).toHaveBeenCalledWith(
      'r1',
      dto,
      actor,
    );
  ...
  it('delegates setFieldPermissions (UC-06-08)', async () => {
    const d = build();
    const dto = { fields: [] };
    await d.controller.setFieldPermissions('r1', dto, actor);
    expect(d.rolesService.setFieldPermissions).toHaveBeenCalledWith(
      'r1',
      dto,
      actor,
    );
