...
function build() {
  ...
  const grantsRepo = { findActive: mockFn(), findById: mockFn(), create: mockFn() };
  const btgRepo = { create: mockFn() };
  const logger = { setContext: mockFn(), info: mockFn(), warn: mockFn() };
  const service = new AuthzClinicalService(em as any, grantsRepo as any, btgRepo as any, logger as any);
...
describe('AuthzClinicalService', () => {
  describe('grantClinicalAccess (UC-06-06)', () => {
    it('grants clinical access for direct treatment without consent', async () => {
      ...
      d.grantsRepo.create.mockReturnValue({ id: 'cag-1', createdAt: new Date() });
      const res = await d.service.grantClinicalAccess(
        'pat-1',
        { grantedUserId: 'u2', tenantId: 't1', purposeOfUse: 'TREATMENT', accessLevel: 'READ', validTo: future } as any,
        actor,
      );
    ...
    it('requires consent for non-treatment purposes', async () => {
      ...
      await expect(
        d.service.grantClinicalAccess(
          'pat-1',
          { grantedUserId: 'u2', tenantId: 't1', purposeOfUse: 'PAYMENT', accessLevel: 'READ', validTo: future } as any,
          actor,
        ),
    ...
    it('rejects a non-positive validity window', async () => {
      ...
      await expect(
        d.service.grantClinicalAccess(
          'pat-1',
          { grantedUserId: 'u2', tenantId: 't1', purposeOfUse: 'TREATMENT', accessLevel: 'READ', validTo: new Date(Date.now() - 1000) } as any,
          actor,
        ),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });
  ...
  describe('breakTheGlass (UC-06-07)', () => {
    it('creates an emergency grant and a break-glass session', async () => {
      const d = build();
      d.grantsRepo.create.mockReturnValue({ id: 'cag-e', createdAt: new Date() });
      const res = await d.service.breakTheGlass(
        'pat-1',
        { tenantId: 't1', justification: 'unconscious patient in ER' } as any,
        actor,
      );
  ...
  describe('revokeClinicalAccess (UC-06-10)', () => {
    it('revokes an active grant', async () => {
      const d = build();
      const grant = { id: 'cag-1', stateConceptId: CONCEPTS.STATE_ACTIVE, validTo: future, updatedAt: new Date() };
    ...
    it('throws when the grant does not exist', async () => {
      ...
      await expect(d.service.revokeClinicalAccess('missing', actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
    ...
    it('rejects revoking a non-active grant', async () => {
      const d = build();
      d.grantsRepo.findById.mockResolvedValue({ id: 'cag-3', stateConceptId: CONCEPTS.STATE_REVOKED });
      await expect(d.service.revokeClinicalAccess('cag-3', actor)).rejects.toBeInstanceOf(
        PreconditionFailedException,
      );
    });
  });
