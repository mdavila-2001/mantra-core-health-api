...
import { PERMISSION_ACTIONS, type PermissionAction } from './create-permission.dto';
import { RESOURCE_TYPES, type ResourceType } from './create-resource-scope-grant.dto';

/** Propósito de uso presentado por el PDP (incluye emergencia). */
export const DECISION_PURPOSES = ['TREATMENT', 'PAYMENT', 'OPERATIONS', 'EMERGENCY'] as const;
...
export class EvaluateDecisionDto {
  ...
  @ApiProperty({ description: 'Recurso protegido (debe casar con permissions.resource)', maxLength: 150 })
  @IsString()
  @MinLength(1)
  @MaxLength(150)
  resource!: string;

  @ApiPropertyOptional({ description: 'Tipo de recurso concreto', enum: RESOURCE_TYPES })
  @IsOptional()
  @IsIn(RESOURCE_TYPES)
  resourceType?: ResourceType;

  @ApiPropertyOptional({ description: 'Id del recurso concreto', format: 'uuid' })
  @IsOptional()
  @IsUUID()
  resourceId?: string;

  @ApiPropertyOptional({ description: 'Perfil de paciente si el recurso es clínico', format: 'uuid' })
  @IsOptional()
  @IsUUID()
  patientProfileId?: string;

  @ApiPropertyOptional({ description: 'Propósito de uso', enum: DECISION_PURPOSES })
  @IsOptional()
