...
function build() {
  ...
  const historyService = { getTimeline: mockFn().mockResolvedValue({ count: 0 }) };
  const thirdPartyService = { record: mockFn().mockResolvedValue({ id: 'del1' }) };
...
describe('AuditController', () => {
  it('delega recordDataAccess (UC-10-01)', async () => {
    ...
    await d.controller.recordDataAccess(dto as any, actor);
  ...
  it('delega getHistory (UC-10-05)', async () => {
    const d = build();
    await d.controller.getHistory('users', 'u1', { as_of: undefined } as any, actor);
    expect(d.historyService.getTimeline).toHaveBeenCalledWith('users', 'u1', { as_of: undefined }, actor);
  });

  it('delega verifyIntegrity (UC-10-06)', async () => {
    const d = build();
    await d.controller.verifyIntegrity({} as any, actor);
    expect(d.eventsService.verifyIntegrity).toHaveBeenCalledWith({}, actor);
  });

  it('delega applyRetention (UC-10-09)', async () => {
    const d = build();
    await d.controller.applyRetention({} as any, actor);
    expect(d.eventsService.applyRetention).toHaveBeenCalledWith({}, actor);
  });

  it('delega scanAnomaly (UC-10-10)', async () => {
    const d = build();
    await d.controller.scanAnomaly({} as any, actor);
  ...
  it('delega recordThirdPartyAccess (UC-10-12)', async () => {
    ...
    await d.controller.recordThirdPartyAccess(dto as any, actor);
