 ...
 */
export const { seeds: AUDIT_CONCEPT_SEEDS, ids: AUD } = defineModuleConcepts('audit', {
  ...
  LEGAL_BASIS_TREATMENT: { code: 'AUDIT_LEGAL_BASIS_TREATMENT', display: 'Treatment legal basis' },
  LEGAL_BASIS_CONSENT: { code: 'AUDIT_LEGAL_BASIS_CONSENT', display: 'Consent legal basis' },
  ...
  RESOURCE_TYPE_CLINICAL: { code: 'AUDIT_RESOURCE_CLINICAL', display: 'Clinical resource' },
  RESOURCE_TYPE_FINANCIAL: { code: 'AUDIT_RESOURCE_FINANCIAL', display: 'Financial resource' },
  RESOURCE_TYPE_ADMIN: { code: 'AUDIT_RESOURCE_ADMIN', display: 'Administrative resource' },

  // --- Propósito de uso (purpose_of_use) ---
  PURPOSE_TREATMENT: { code: 'AUDIT_PURPOSE_TREATMENT', display: 'Treatment' },
  PURPOSE_PAYMENT: { code: 'AUDIT_PURPOSE_PAYMENT', display: 'Payment' },
  PURPOSE_OPERATIONS: { code: 'AUDIT_PURPOSE_OPERATIONS', display: 'Healthcare operations' },
  PURPOSE_COVERAGE: { code: 'AUDIT_PURPOSE_COVERAGE', display: 'Coverage / insurance' },
  PURPOSE_VERIFICATION: { code: 'AUDIT_PURPOSE_VERIFICATION', display: 'Identity verification' },
  ...
  GOVERNANCE_EXPORT: { code: 'AUDIT_GOV_EXPORT', display: 'Governance export' },
  GOVERNANCE_DISCLOSURE: { code: 'AUDIT_GOV_DISCLOSURE', display: 'Governance disclosure' },
  ...
  APPROVAL_PENDING: { code: 'AUDIT_APPROVAL_PENDING', display: 'Pending approval' },
  APPROVAL_REJECTED: { code: 'AUDIT_APPROVAL_REJECTED', display: 'Rejected' },

  // --- DSAR: tipos y estados (dsar_requests) ---
  DSAR_TYPE_ACCESS: { code: 'AUDIT_DSAR_TYPE_ACCESS', display: 'Access request' },
  DSAR_TYPE_ERASURE: { code: 'AUDIT_DSAR_TYPE_ERASURE', display: 'Erasure request' },
  DSAR_TYPE_RECTIFICATION: { code: 'AUDIT_DSAR_TYPE_RECTIFICATION', display: 'Rectification request' },
  DSAR_TYPE_PORTABILITY: { code: 'AUDIT_DSAR_TYPE_PORTABILITY', display: 'Portability request' },
  DSAR_TYPE_OBJECTION: { code: 'AUDIT_DSAR_TYPE_OBJECTION', display: 'Objection request' },
  DSAR_RECEIVED: { code: 'AUDIT_DSAR_STATUS_RECEIVED', display: 'Received' },
  DSAR_IN_PROGRESS: { code: 'AUDIT_DSAR_STATUS_IN_PROGRESS', display: 'In progress' },
  DSAR_COMPLETED: { code: 'AUDIT_DSAR_STATUS_COMPLETED', display: 'Completed' },
  DSAR_REJECTED: { code: 'AUDIT_DSAR_STATUS_REJECTED', display: 'Rejected' },

  // --- Jurisdicciones ---
  JURISDICTION_PE: { code: 'AUDIT_JURISDICTION_PE', display: 'Peru' },
  JURISDICTION_EU: { code: 'AUDIT_JURISDICTION_EU', display: 'European Union' },
  JURISDICTION_US: { code: 'AUDIT_JURISDICTION_US', display: 'United States' },

  // --- Moderación (moderation_events): tipos de destino ---
  MOD_TARGET_CONTENT: { code: 'AUDIT_MOD_TARGET_CONTENT', display: 'Content target' },
  MOD_TARGET_USER: { code: 'AUDIT_MOD_TARGET_USER', display: 'User target' },
  MOD_TARGET_COMMENT: { code: 'AUDIT_MOD_TARGET_COMMENT', display: 'Comment target' },
  MOD_TARGET_REVIEW: { code: 'AUDIT_MOD_TARGET_REVIEW', display: 'Review target' },

  // --- Moderación: acciones ---
  MOD_ACTION_REMOVE: { code: 'AUDIT_MOD_ACTION_REMOVE', display: 'Remove' },
  MOD_ACTION_FLAG: { code: 'AUDIT_MOD_ACTION_FLAG', display: 'Flag' },
  MOD_ACTION_APPROVE: { code: 'AUDIT_MOD_ACTION_APPROVE', display: 'Approve' },
  MOD_ACTION_RESTRICT: { code: 'AUDIT_MOD_ACTION_RESTRICT', display: 'Restrict' },
  MOD_ACTION_DISMISS: { code: 'AUDIT_MOD_ACTION_DISMISS', display: 'Dismiss' },

  // --- Moderación: razones ---
  MOD_REASON_POLICY: { code: 'AUDIT_MOD_REASON_POLICY', display: 'Policy violation' },
  MOD_REASON_ABUSE: { code: 'AUDIT_MOD_REASON_ABUSE', display: 'Abuse' },
  MOD_REASON_SPAM: { code: 'AUDIT_MOD_REASON_SPAM', display: 'Spam' },
  MOD_REASON_LEGAL: { code: 'AUDIT_MOD_REASON_LEGAL', display: 'Legal request' },
