...
/**
 * Mapea la entidad persistente asociada a `provider_protocol_configs_history`.
 */
@Entity({ schema: 'audit', tableName: 'provider_protocol_configs_history' })
export class ProviderProtocolConfigsHistory {
  /**
  ...
  /**
   * Identificador asociado a provider protocol config.
   */
  @Property({ fieldName: 'provider_protocol_config_id', type: 'uuid' }) // FK → auth_providers.provider_protocol_configs
  providerProtocolConfigId!: string;
