...
describe('TenantsController', () => {
  it('delegates createChild (UC-04-03)', async () => {
    ...
    await d.controller.createChild('t1', dto as any, actor);
  ...
  it('delegates createBranch (UC-04-04)', async () => {
    ...
    await d.controller.createBranch('t1', dto as any, actor);
  ...
  it('delegates invite (UC-04-05)', async () => {
    const d = build();
    const dto = { userId: 'u1' };
    await d.controller.invite('t1', dto as any, actor);
  ...
  it('delegates assignBranch (UC-04-06)', async () => {
    ...
    await d.controller.assignBranch('t1', 'm1', dto as any, actor);
    expect(d.membershipsService.assignBranch).toHaveBeenCalledWith('t1', 'm1', dto, actor);
  });

  it('delegates transfer (UC-04-07)', async () => {
    const d = build();
    const dto = { fromBranchId: 'b1', toBranchId: 'b2' };
    await d.controller.transfer('t1', 'm1', dto as any, actor);
    expect(d.membershipsService.transfer).toHaveBeenCalledWith('t1', 'm1', dto, actor);
  });
  ...
  it('delegates changeRole (UC-04-08)', async () => {
    ...
    expect(d.membershipsService.changeRole).toHaveBeenCalledWith('t1', 'm1', dto, actor);
  });

  it('delegates offboard (UC-04-09)', async () => {
    const d = build();
    await d.controller.offboard('t1', 'm1', actor);
    expect(d.membershipsService.offboard).toHaveBeenCalledWith('t1', 'm1', actor);
  });
});
