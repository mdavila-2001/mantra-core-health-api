...
function build() {
  ...
  const service = new DiagnosticsReportsService(em as any, repo as any, logger as any);
...
describe('DiagnosticsReportsService', () => {
  describe('createReportVersion (UC-20-07)', () => {
    it('requires custodian tenant', async () => {
      const d = build();
      await expect(d.service.createReportVersion('r1', {} as any, actor)).rejects.toBeInstanceOf(
        PreconditionFailedException,
      );
    });
    ...
    it('creates version numbered next and links results', async () => {
      ...
      d.repo.createVersion.mockReturnValue({ id: 'v3', clinicalStatusConceptId: DIAG.REPORT_PRELIMINARY });

      const res = await d.service.createReportVersion(
        'r1',
        { custodianTenantId: 't1', results: [{ observationId: 'o1' }] } as any,
        actor,
      );
      ...
      expect(d.repo.createVersion).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ versionNumber: 3, releaseEligibilityConceptId: DIAG.RELEASE_ELIGIBLE }),
      );
    ...
    it('throws when superseded version not in report', async () => {
      ...
      await expect(
        d.service.createReportVersion('r1', { custodianTenantId: 't1', supersedesVersionId: 'vX' } as any, actor),
  ...
  describe('releaseVersion (UC-20-08)', () => {
    it('throws when version missing', async () => {
      const d = build();
      d.repo.findVersionInReport.mockResolvedValue(null);
      await expect(d.service.releaseVersion('r1', 'v1', {} as any, actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
    ...
    it('releases an eligible preliminary version', async () => {
      ...
      const res = await d.service.releaseVersion('r1', 'v1', {} as any, actor);
  ...
  describe('detectCritical (UC-20-09)', () => {
    it('conflicts on duplicate critical for observation', async () => {
      ...
      await expect(
        d.service.detectCritical({ observationId: 'o1', patientProfileId: 'p1', custodianTenantId: 't1' } as any, actor),
      ).rejects.toBeInstanceOf(ConflictException);
    });

    it('creates a pending notification', async () => {
      const d = build();
      d.repo.createCriticalNotification.mockReturnValue({ id: 'n1', notificationStatusConceptId: DIAG.CRITICAL_PENDING });
      const res = await d.service.detectCritical(
        { observationId: 'o1', patientProfileId: 'p1', custodianTenantId: 't1' } as any,
        actor,
      );
  ...
  describe('acknowledgeCritical (UC-20-10)', () => {
    it('escalates when SLA elapsed', async () => {
      ...
      const res = await d.service.acknowledgeCritical('n1', { acknowledgedByProfileId: 'p1' } as any, actor);
