 ...
 */
export const { seeds: DIAGNOSTICS_CONCEPT_SEEDS, ids: DIAG } = defineModuleConcepts('diagnostics', {
  // --- Ciclo de vida del espécimen (specimens.status_concept_id) ---
  SPECIMEN_COLLECTED: { code: 'SPEC_COLLECTED', display: 'Specimen collected' },
  ...
  ACCESSION_IN_PROCESS: { code: 'ACC_IN_PROCESS', display: 'Accession in process' },

  // --- Item de acesión (accession_specimens.status_concept_id) ---
  ACCESSION_ITEM_RECEIVED: { code: 'ACC_ITEM_RECEIVED', display: 'Accession item received' },
  ACCESSION_ITEM_REJECTED: { code: 'ACC_ITEM_REJECTED', display: 'Accession item rejected' },
  ...
  CUSTODY_RECEPTION: { code: 'CUSTODY_RECEPTION', display: 'Custody reception' },
  ...
  CONTAINER_IN_TRANSIT: { code: 'CONTAINER_IN_TRANSIT', display: 'Container in transit' },
  CONTAINER_STORED: { code: 'CONTAINER_STORED', display: 'Container stored' },
  CONTAINER_EVENT_TRANSFER: { code: 'CONTAINER_EVT_TRANSFER', display: 'Container transfer event' },

  // --- Motivo de rechazo por defecto (specimen_rejection_events) ---
  REJECTION_REASON_QUALITY: { code: 'REJECTION_QUALITY', display: 'Rejected for quality (hemolysis/volume)' },
  ...
  TEST_PRELIMINARY: { code: 'TEST_PRELIMINARY', display: 'Test preliminary result' },
  ...
  VERIFIABLE_OBSERVATION: { code: 'VERIFIABLE_OBSERVATION', display: 'Verifiable observation' },
  VERIFICATION_TECHNICAL: { code: 'VERIF_TECHNICAL', display: 'Technical verification level' },
  VERIFICATION_MEDICAL: { code: 'VERIF_MEDICAL', display: 'Faculty verification level' },
  VERIFICATION_ACCEPTED: { code: 'VERIF_ACCEPTED', display: 'Verification accepted' },

  // --- Informe diagnóstico (diagnostic_report_versions.clinical_status_concept_id) ---
  REPORT_PRELIMINARY: { code: 'REPORT_PRELIMINARY', display: 'Report preliminary' },
  ...
  RELEASE_ACTION_RELEASE: { code: 'RELEASE_ACTION', display: 'Release action' },
  VISIBILITY_PATIENT_VISIBLE: { code: 'PATIENT_VISIBLE', display: 'Visible to patient' },
  VISIBILITY_PATIENT_HIDDEN: { code: 'PATIENT_HIDDEN', display: 'Hidden from patient' },

  // --- Resultado crítico (critical_result_notifications) ---
  CRITICALITY_CRITICAL: { code: 'CRITICALITY_CRITICAL', display: 'Critical value' },
  CRITICAL_PENDING: { code: 'CRITICAL_PENDING', display: 'Critical notification pending' },
  CRITICAL_ACKNOWLEDGED: { code: 'CRITICAL_ACK', display: 'Critical notification acknowledged' },
  CRITICAL_ESCALATED: { code: 'CRITICAL_ESCALATED', display: 'Critical notification escalated' },

  // --- Imagen: endpoint DICOM (imaging_endpoints) ---
  IMAGING_ENDPOINT_STOW: { code: 'ENDPOINT_STOW', display: 'STOW-RS endpoint type' },
  IMAGING_ENDPOINT_ACTIVE: { code: 'ENDPOINT_ACTIVE', display: 'Imaging endpoint active' },

  // --- Imagen: estudio/serie/instancia (imaging_studies, series, instances) ---
  IMAGING_STUDY_STORED: { code: 'STUDY_STORED', display: 'Imaging study stored' },
  IMAGING_MODALITY_OTHER: { code: 'MODALITY_OTHER', display: 'Imaging modality (unspecified)' },
  SOP_CLASS_OTHER: { code: 'SOP_CLASS_OTHER', display: 'SOP class (unspecified)' },
  OBJECT_LOCATION_ACTIVE: { code: 'OBJLOC_ACTIVE', display: 'Object location active' },
  PROCEDURE_STEP_COMPLETED: { code: 'MPPS_COMPLETED', display: 'Procedure step completed' },

  // --- Media clínica (clinical_media, media_annotations) ---
  MEDIA_TYPE_PHOTO: { code: 'MEDIA_PHOTO', display: 'Clinical photograph' },
  MEDIA_STATUS_ACTIVE: { code: 'MEDIA_ACTIVE', display: 'Clinical media active' },
  ANNOTATION_TYPE_MANUAL: { code: 'ANNOTATION_MANUAL', display: 'Manual annotation' },
  ANNOTATION_STATUS_ACTIVE: { code: 'ANNOTATION_ACTIVE', display: 'Annotation active' },
  ...
  DQ_TARGET_SPECIMEN: { code: 'DQ_TARGET_SPECIMEN', display: 'Data-quality target: specimen' },
  DQ_SEVERITY_WARNING: { code: 'DQ_SEVERITY_WARNING', display: 'Data-quality warning' },
  DQ_OPEN: { code: 'DQ_OPEN', display: 'Data-quality event open' },
  DQ_RESOLVED: { code: 'DQ_RESOLVED', display: 'Data-quality event resolved' },
  PROVENANCE_TARGET: { code: 'PROV_TARGET', display: 'Provenance target' },
  PROVENANCE_SOURCE: { code: 'PROV_SOURCE', display: 'Provenance source' },
  PROVENANCE_DERIVATION: { code: 'PROV_DERIVATION', display: 'Provenance derivation activity' },
});
