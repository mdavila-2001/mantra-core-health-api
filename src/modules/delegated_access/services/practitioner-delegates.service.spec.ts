...
function build() {
  ...
  const grantsRepo = { create: mockFn(), revokeActiveByAssignment: mockFn().mockResolvedValue(0) };
  const requestsRepo = { cancelOpenByAssignment: mockFn().mockResolvedValue(0) };
  ...
  return { service, tx, delegatesRepo, orgAssignmentsRepo, setsRepo, grantsRepo, requestsRepo, eventsRepo };
}
...
describe('PractitionerDelegatesService', () => {
  describe('createDelegate (UC-29-03)', () => {
    it('creates the delegation and records DELEGATION_CREATED', async () => {
      const d = build();
      d.orgAssignmentsRepo.findById.mockResolvedValue({ id: 'org1', statusConceptId: STATUS.ACTIVE });
      d.setsRepo.findById.mockResolvedValue({ id: 'set1', statusConceptId: STATUS.ACTIVE });
      d.delegatesRepo.create.mockReturnValue({ id: 'del1', statusConceptId: STATUS.ACTIVE, createdAt: new Date() });
      const res = await d.service.createDelegate(createDto as any, actor);
      expect(res.id).toBe('del1');
      expect(d.tx.flush).toHaveBeenCalled();
      expect(d.eventsRepo.record).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ eventTypeConceptId: DELEG.EVENT_DELEGATION_CREATED }),
      );
    });
    ...
    it('throws when the org user assignment is missing', async () => {
      ...
      await expect(d.service.createDelegate(createDto as any, actor)).rejects.toBeInstanceOf(
    ...
    it('rejects when the permission set is not active (precondition)', async () => {
      const d = build();
      d.orgAssignmentsRepo.findById.mockResolvedValue({ id: 'org1', statusConceptId: STATUS.ACTIVE });
      d.setsRepo.findById.mockResolvedValue({ id: 'set1', statusConceptId: STATUS.REVOKED });
      await expect(d.service.createDelegate(createDto as any, actor)).rejects.toBeInstanceOf(
    ...
    it('never delegates signing clinical content', async () => {
      ...
      d.setsRepo.findById.mockResolvedValue({ id: 'set1', statusConceptId: STATUS.ACTIVE });
      d.delegatesRepo.create.mockReturnValue({ id: 'del1', statusConceptId: STATUS.ACTIVE, createdAt: new Date() });
      await d.service.createDelegate(createDto as any, actor);
  ...
  describe('issueGrant (UC-29-06)', () => {
    it('rejects when the delegation is not active', async () => {
      ...
      await expect(
        d.service.issueGrant('del1', { purpose: 'TREATMENT', validTo: '2027-01-01T00:00:00Z' } as any, actor),
    ...
    it('issues a grant and records GRANT_ISSUED', async () => {
      ...
      d.grantsRepo.create.mockReturnValue({ id: 'g1', statusConceptId: STATUS.ACTIVE, createdAt: new Date() });
      ...
      expect(d.eventsRepo.record).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ eventTypeConceptId: DELEG.EVENT_GRANT_ISSUED }),
      );
    });
  });
  ...
  describe('revoke (UC-29-07)', () => {
    it('throws when the delegation does not exist', async () => {
      ...
      await expect(d.service.revoke('del1', {} as any, actor)).rejects.toBeInstanceOf(
        ResourceNotFoundException,
      );
    });
    ...
    it('is idempotent when already revoked', async () => {
      const d = build();
      d.delegatesRepo.findById.mockResolvedValue({ id: 'del1', statusConceptId: STATUS.REVOKED });
      const res = await d.service.revoke('del1', {} as any, actor);
    ...
    it('revokes the delegation, cascades grants and cancels open requests', async () => {
      ...
      const res = await d.service.revoke('del1', { reason: 'abuse' } as any, actor);
      expect(res).toEqual({ ok: true });
      ...
      expect(d.eventsRepo.record).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ eventTypeConceptId: DELEG.EVENT_DELEGATION_REVOKED }),
      );
    });
  });
});
