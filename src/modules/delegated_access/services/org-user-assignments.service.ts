...
import { ACCESS_SCOPE_CONCEPT, ASSIGNMENT_ROLE_CONCEPT, STATUS } from './concept-maps';
...
export class OrgUserAssignmentsService {
  ...
  async createAssignment(
    ...
    this.logger.info(
      { operation: 'delegated_access.org_assignment.create', tenantMembershipId, actorId: actor.id },
    ...
    return this.em.transactional(async (tx) => {
      ...
      if (overlap) {
        this.logger.warn(
          { operation: 'delegated_access.org_assignment.create', reason: 'overlap' },
          'Rejected: active role+scope assignment already exists',
        );
        throw new ConflictException('Ya existe una asignación activa con ese rol y alcance', {
          tenantMembershipId,
        });
      }
      ...
      this.logger.info(
        { operation: 'delegated_access.org_assignment.create', assignmentId: assignment.id },
        'Organization user assignment created',
      );
      return { id: assignment.id, status: assignment.statusConceptId, createdAt: assignment.createdAt };
    });
  }
  ...
  async updateAssignment(
    ...
    this.logger.info(
      { operation: 'delegated_access.org_assignment.update', assignmentId: id, actorId: actor.id },
      'Updating organization user assignment',
    );
    if (dto.supervisorUserId === undefined && dto.accessScope === undefined && !dto.suspend) {
      throw new PreconditionFailedException('No hay cambios que aplicar', { assignmentId: id });
    }

    return this.em.transactional(async (tx) => {
      const assignment = await this.assignmentsRepo.findById(tx, id);
      if (!assignment) {
        throw new ResourceNotFoundException('Asignación no encontrada', { assignmentId: id });
      }
      if (assignment.statusConceptId !== STATUS.ACTIVE) {
        throw new PreconditionFailedException('La asignación no está activa', { assignmentId: id });
      }
      if (
      ...
      if (dto.supervisorUserId !== undefined) assignment.supervisorUserId = dto.supervisorUserId;
      ...
      this.logger.info(
        { operation: 'delegated_access.org_assignment.update', assignmentId: id, suspended: !!dto.suspend },
