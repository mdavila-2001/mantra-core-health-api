...
function build() {
  const delegatesService = { createDelegate: mockFn(), issueGrant: mockFn(), revoke: mockFn() };
  const requestsService = { requestAccess: mockFn() };
  const controller = new PractitionerDelegatesController(delegatesService as any, requestsService as any);
...
describe('PractitionerDelegatesController', () => {
  it('delegates create (UC-29-03)', async () => {
    const d = build();
    const dto = { practitionerRoleAssignmentId: 'a', delegateUserAssignmentId: 'b', delegatedPermissionSetId: 'c' };
  ...
  it('delegates requestAccess (UC-29-04)', async () => {
    ...
    await d.controller.requestAccess('del1', dto as any, actor);
    expect(d.requestsService.requestAccess).toHaveBeenCalledWith('del1', dto, actor);
  });
  ...
  it('delegates issueGrant (UC-29-06)', async () => {
    ...
    expect(d.delegatesService.issueGrant).toHaveBeenCalledWith('del1', dto, actor);
  });

  it('delegates revoke (UC-29-07)', async () => {
    const d = build();
    await d.controller.revoke('del1', { reason: 'x' } as any, actor);
    expect(d.delegatesService.revoke).toHaveBeenCalledWith('del1', { reason: 'x' }, actor);
  });
});
