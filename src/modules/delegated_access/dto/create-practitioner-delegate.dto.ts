...
export class CreatePractitionerDelegateDto {
  @ApiProperty({ description: 'Asignación de rol del practitioner delegante', format: 'uuid' })
  @IsUUID()
  practitionerRoleAssignmentId!: string;

  @ApiProperty({ description: 'Asignación de usuario de organización (delegado)', format: 'uuid' })
  @IsUUID()
  delegateUserAssignmentId!: string;

  @ApiProperty({ description: 'Set de permisos delegados a aplicar', format: 'uuid' })
  @IsUUID()
  delegatedPermissionSetId!: string;

  @ApiPropertyOptional({ description: 'Rol del delegado', enum: DELEGATE_ROLES })
  @IsOptional()
  @IsIn(DELEGATE_ROLES as unknown as string[])
  delegateRole?: (typeof DELEGATE_ROLES)[number];

  @ApiPropertyOptional({ description: 'Alcance de pacientes', enum: PATIENT_SCOPES })
  @IsOptional()
  @IsIn(PATIENT_SCOPES as unknown as string[])
  patientScope?: (typeof PATIENT_SCOPES)[number];

  @ApiPropertyOptional({ description: 'Alcance de citas', enum: APPOINTMENT_SCOPES })
  @IsOptional()
  @IsIn(APPOINTMENT_SCOPES as unknown as string[])
