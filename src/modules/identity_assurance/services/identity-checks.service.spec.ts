...
describe('IdentityChecksService', () => {
  describe('recordAttempt (UC-27-05)', () => {
    it('numbers the attempt per case and moves the check to in-progress', async () => {
      const d = build();
      const check: any = { id: 'ch1', identityVerificationCaseId: 'k1', statusConceptId: IDA.CHECK_PENDING, updatedAt: new Date() };
      d.checksRepo.findById.mockResolvedValue(check);
      d.attemptsRepo.countByCase.mockResolvedValue(2);
      d.attemptsRepo.create.mockReturnValue({ id: 'at1', attemptNumber: 3, outcomeConceptId: IDA.ATTEMPT_SUCCESS });
      const res = await d.service.recordAttempt('ch1', { identityAuthorityEndpointId: 'ep1' } as any, actor);
      expect(res.attemptNumber).toBe(3);
      expect(check.statusConceptId).toBe(IDA.CHECK_IN_PROGRESS);
      expect(d.attemptsRepo.create).toHaveBeenCalledWith(d.tx, expect.objectContaining({ attemptNumber: 3 }));
    });
    ...
    it('throws when the check does not exist', async () => {
      ...
      await expect(
        d.service.recordAttempt('missing', { identityAuthorityEndpointId: 'ep1' } as any, actor),
  ...
  describe('recordResult (UC-27-06)', () => {
    it('requires a completed attempt (precondition)', async () => {
      const d = build();
      d.checksRepo.findById.mockResolvedValue({ id: 'ch1', identityVerificationCaseId: 'k1' });
    ...
    it('appends a versioned result chaining supersede and completes the check', async () => {
      const d = build();
      const check: any = { id: 'ch1', identityVerificationCaseId: 'k1', statusConceptId: IDA.CHECK_IN_PROGRESS, updatedAt: new Date() };
      d.checksRepo.findById.mockResolvedValue(check);
      d.resultsRepo.findLatestByCheck.mockResolvedValue({ id: 'r0' });
      d.resultsRepo.countByCheck.mockResolvedValue(1);
      d.resultsRepo.create.mockReturnValue({ id: 'r1', resultVersion: 2, resultConceptId: IDA.RESULT_MATCH });
      const res = await d.service.recordResult('ch1', { result: 'MATCH' } as any, actor);
      expect(res.resultVersion).toBe(2);
      expect(check.statusConceptId).toBe(IDA.CHECK_COMPLETED);
      expect(d.resultsRepo.create).toHaveBeenCalledWith(d.tx, expect.objectContaining({ supersedesResultId: 'r0', resultVersion: 2 }));
    });
  });
});
