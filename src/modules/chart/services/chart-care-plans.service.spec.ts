...
function build() {
  ...
  const service = new ChartCarePlansService(em as any, carePlansRepo as any, logger as any);
...
describe('ChartCarePlansService', () => {
  describe('createCarePlan (UC-15-10)', () => {
    it('flushes the plan before its activities and defaults the activity concept', async () => {
      const d = build();
      d.carePlansRepo.createPlan.mockReturnValue({ id: 'cp1', statusConceptId: CHART.CAREPLAN_ACTIVE, createdAt: new Date() });

      const res = await d.service.createCarePlan(
        { patientProfileId: 'p1', activities: [{ detailText: 'walk' }] } as any,
        actor,
      );
      ...
      expect(d.carePlansRepo.createActivity).toHaveBeenCalledWith(
        d.tx,
        expect.objectContaining({ activityConceptId: CHART.ACTIVITY_DEFAULT, statusConceptId: CHART.ACTIVITY_SCHEDULED }),
      );
    });
  });
  ...
  describe('updateActivity (UC-15-11)', () => {
    it('throws when the plan is not active', async () => {
      const d = build();
      d.carePlansRepo.findPlanById.mockResolvedValue({ id: 'cp1', statusConceptId: CHART.CAREPLAN_COMPLETED });
      await expect(
        d.service.updateActivity('cp1', 'a1', { status: 'COMPLETED' } as any, actor),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });

    it('throws when the activity does not belong to the plan', async () => {
      const d = build();
      d.carePlansRepo.findPlanById.mockResolvedValue({ id: 'cp1', statusConceptId: CHART.CAREPLAN_ACTIVE });
      d.carePlansRepo.findActivityById.mockResolvedValue({ id: 'a1', carePlanId: 'other' });
      await expect(
        d.service.updateActivity('cp1', 'a1', { status: 'COMPLETED' } as any, actor),
    ...
    it('completes the plan when all activities are completed', async () => {
      const d = build();
      const plan: any = { id: 'cp1', statusConceptId: CHART.CAREPLAN_ACTIVE, updatedAt: new Date() };
      const activity: any = { id: 'a1', carePlanId: 'cp1', statusConceptId: CHART.ACTIVITY_SCHEDULED, updatedAt: new Date() };
      ...
      const res = await d.service.updateActivity('cp1', 'a1', { status: 'COMPLETED', detailText: 'done' } as any, actor);
    ...
    it('keeps the plan active when some activities are not completed', async () => {
      ...
      const res = await d.service.updateActivity('cp1', 'a1', { status: 'IN_PROGRESS' } as any, actor);
