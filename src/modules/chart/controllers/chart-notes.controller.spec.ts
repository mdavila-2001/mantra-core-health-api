...
describe('ChartNotesController', () => {
  it('delegates createNote (UC-15-01)', async () => {
    ...
    await d.controller.createNote(dto as any, actor);
  ...
  it('delegates addVersion (UC-15-02)', async () => {
    ...
    await d.controller.addVersion('n1', dto as any, actor);
  ...
  it('delegates signVersion (UC-15-03)', async () => {
    ...
    await d.controller.signVersion('n1', 'v1', dto as any, actor);
    expect(d.notesService.signVersion).toHaveBeenCalledWith('n1', 'v1', dto, actor);
  });

  it('delegates cosignVersion (UC-15-04)', async () => {
    const d = build();
    const dto = { signerProfileId: 's2' };
    await d.controller.cosignVersion('n1', 'v1', dto as any, actor);
    expect(d.notesService.cosignVersion).toHaveBeenCalledWith('n1', 'v1', dto, actor);
  });

  it('delegates amendNote (UC-15-05)', async () => {
    const d = build();
    const dto = { authorProfileId: 'a1', amendmentReasonText: 'x' };
    await d.controller.amendNote('n1', dto as any, actor);
    expect(d.notesService.amendNote).toHaveBeenCalledWith('n1', dto, actor);
  });

  it('delegates releaseVersion (UC-15-06)', async () => {
    const d = build();
    await d.controller.releaseVersion('v1', {} as any, actor);
    expect(d.notesService.releaseVersion).toHaveBeenCalledWith('v1', {}, actor);
  });

  it('delegates withholdVersion (UC-15-07)', async () => {
    const d = build();
    await d.controller.withholdVersion('v1', {} as any, actor);
    expect(d.notesService.withholdVersion).toHaveBeenCalledWith('v1', {}, actor);
  });

  it('delegates recordExamFindings (UC-15-08)', async () => {
    const d = build();
    const dto = { findings: [{}] };
    await d.controller.recordExamFindings('v1', dto as any, actor);
    expect(d.notesService.recordExamFindings).toHaveBeenCalledWith('v1', dto, actor);
  });
});
