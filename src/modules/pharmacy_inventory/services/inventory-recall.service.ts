...
export class InventoryRecallService {
  ...
  async createHold(
    dto: CreateRecallHoldDto,
    actor: AuthenticatedUser,
  ): Promise<IdResponseDto> {
    this.logger.info(
      {
        operation: 'pharmacy_inventory.recall.create',
        lotId: dto.inventoryLotId,
        ref: dto.recallReference,
      },
    ...
    return this.em.transactional(async (tx) => {
      ...
      if (position) {
        position.quarantineQuantity = String(
          num(position.quarantineQuantity) + onHand,
        );
  ...
  async release(
    id: string,
    dto: ReleaseRecallHoldDto,
    actor: AuthenticatedUser,
  ): Promise<StatusResultDto> {
    this.logger.info(
      { operation: 'pharmacy_inventory.recall.release', holdId: id },
      'Releasing recall hold',
    );
    return this.em.transactional(async (tx) => {
      const hold = await this.holdsRepo.findById(tx, id);
      if (!hold) {
        throw new ResourceNotFoundException('Hold de recall no encontrado', {
          holdId: id,
        });
      }
      if (hold.holdStatusConceptId !== PINV.HOLD_ACTIVE) {
        throw new PreconditionFailedException('El hold no está activo', {
          holdId: id,
        });
      }
      ...
      if (hold.inventoryLotId) {
        ...
        if (lot) {
          ...
          const positions = await this.stockRepo.findByLot(
            tx,
            hold.inventoryLotId,
          );
          for (const pos of positions) {
            ...
            this.ledgerRepo.append(tx, {
              ...
              ledgerSequence: sequence,
              movementTypeConceptId: writeOff
                ? PINV.MV_WRITE_OFF
                : PINV.MV_QUARANTINE_RELEASE,
            ...
            if (writeOff)
              pos.onHandQuantity = String(
                num(pos.onHandQuantity) - quarantined,
              );
            pos.availableQuantity = recompute(
              pos.onHandQuantity,
              pos.reservedQuantity,
              pos.quarantineQuantity,
            );
