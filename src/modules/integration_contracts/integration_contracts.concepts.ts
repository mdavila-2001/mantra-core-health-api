...
export const { seeds: INTEGRATION_CONTRACTS_CONCEPT_SEEDS, ids: ICON } =
  defineModuleConcepts('integration_contracts', {
    ...
    VERSION_ACTIVE: {
      code: 'VERSION_ACTIVE',
      display: 'Contract version active',
    },
    VERSION_SUPERSEDED: {
      code: 'VERSION_SUPERSEDED',
      display: 'Contract version superseded',
    },

    // Capacidad / clasificación por defecto del contrato
    CAPABILITY_GENERIC: {
      code: 'CAPABILITY_GENERIC',
      display: 'Generic data exchange capability',
    },

    // Perfil de autenticación
    AUTH_OAUTH2_CONFIDENTIAL: {
      code: 'OAUTH2_CONFIDENTIAL',
      display: 'OAuth2 confidential client',
    },
    AUTH_PROFILE_ACTIVE: {
      code: 'AUTH_ACTIVE',
      display: 'Auth profile active',
    },
    AUTH_PROFILE_ROTATED: {
      code: 'AUTH_ROTATED',
      display: 'Auth profile rotated',
    },
    AUTH_PROFILE_REVOKED: {
      code: 'AUTH_REVOKED',
      display: 'Auth profile revoked',
    },
    ...
    WEBHOOK_EVENT_GENERIC: {
      code: 'WEBHOOK_EVENT_GENERIC',
      display: 'Generic webhook event type',
    },
    SUBSCRIPTION_ACTIVE: {
    ...
    SUBSCRIPTION_SUSPENDED: {
      ...
      display: 'Webhook subscription suspended',
    },
    ...
    OUTCOME_DEAD_LETTER: {
      code: 'DEAD_LETTER',
      display: 'Exchange dead-lettered',
    },
    ...
    // Idempotencia
    OPERATION_EXCHANGE: {
      code: 'OPERATION_EXCHANGE',
      display: 'Idempotent exchange operation',
    },
    IDEMPOTENCY_PENDING: {
      code: 'IDEMPOTENCY_PENDING',
      display: 'Idempotency pending',
    },
    IDEMPOTENCY_COMPLETED: {
      code: 'IDEMPOTENCY_COMPLETED',
      display: 'Idempotency completed',
    },
    ...
    DELIVERY_FAILED: {
      ...
      display: 'Webhook delivery failed',
    },
  });
