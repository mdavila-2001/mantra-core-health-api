...
function build() {
  ...
  const pageViewsRepo = { findById: mockFn(), findByRouteAndCode: mockFn(), create: mockFn() };
  const childrenRepo = {
    listFields: mockFn(),
    listActions: mockFn(),
    createField: mockFn(),
  ...
  return {
    service, tx, em, definitionsRepo, runsRepo, surfacesRepo, routesRepo,
    pageViewsRepo, childrenRepo, prefsRepo,
  };
}
...
const publishDto = {
  ...
  fields: [{ fieldCode: 'name', sourceColumn: 'name', label: 'Name', dataType: 'string', ordinal: 1 }],
  actions: [{ actionCode: 'view', label: 'View', actionType: 'NAVIGATE' as const, ordinal: 1 }],
...
describe('FrontendViewsService', () => {
  describe('publishViewContract (UC-30-02)', () => {
    it('rejects when the definition does not exist (404)', async () => {
      ...
      await expect(
        d.service.publishViewContract('portal', 'route', publishDto as any, admin),
      ).rejects.toBeInstanceOf(ResourceNotFoundException);
    });

    it('rejects when the definition is not ACTIVE (422)', async () => {
      const d = build();
      d.definitionsRepo.findById.mockResolvedValue({ id: 'def-1', statusConceptId: RM.DEF_DRAFT });
      await expect(
        d.service.publishViewContract('portal', 'route', publishDto as any, admin),
    ...
    it('upserts surface/route, creates the view and its children', async () => {
      ...
      const res = await d.service.publishViewContract('portal', 'route', publishDto as any, admin);
  ...
  describe('serveData (UC-30-05 / UC-30-11)', () => {
    it('masks sensitive fields for a non-privileged user and computes staleness', async () => {
      ...
      d.childrenRepo.listFields.mockResolvedValue([
        { fieldCode: 'name', label: 'Name', dataType: 'string', sensitive: false },
        { fieldCode: 'ssn', label: 'SSN', dataType: 'string', sensitive: true },
      ]);
      d.childrenRepo.listActions.mockResolvedValue([
        { actionCode: 'edit', label: 'Edit', actionTypeConceptId: RM.ACTION_TYPE_MUTATION, requiredPermissionId: 'perm-1' },
      ]);
      d.runsRepo.findLatestByDefinition.mockResolvedValue({ completedAt: new Date(Date.now() - 5000) });

      const res = await d.service.serveData('portal', 'route', 'account_list', endUser);

      expect(res.fields.find((f) => f.fieldCode === 'ssn')?.masked).toBe(true);
      expect(res.fields.find((f) => f.fieldCode === 'name')?.masked).toBe(false);
  ...
  describe('upsertPreferences (UC-30-09)', () => {
    it('rejects visible fields outside the contract allow-list (422)', async () => {
      const d = build();
      d.pageViewsRepo.findById.mockResolvedValue({ id: 'view-1', statusConceptId: CONCEPTS.STATE_ACTIVE });
      d.childrenRepo.listFields.mockResolvedValue([{ fieldCode: 'name' }]);

      await expect(
        d.service.upsertPreferences('view-1', { visibleFields: ['name', 'hacker'] } as any, endUser),
      ).rejects.toBeInstanceOf(PreconditionFailedException);
    });
    ...
    it('creates preferences when none exist', async () => {
      ...
      d.prefsRepo.create.mockReturnValue({ id: 'pref-1', statusConceptId: CONCEPTS.STATE_ACTIVE });
    ...
    it('updates existing preferences in place', async () => {
      const d = build();
      d.pageViewsRepo.findById.mockResolvedValue({ id: 'view-1', statusConceptId: CONCEPTS.STATE_ACTIVE });
      const existing = { id: 'pref-1', statusConceptId: CONCEPTS.STATE_ACTIVE, updatedAt: new Date() };
      d.prefsRepo.findByUserAndView.mockResolvedValue(existing);

      const res = await d.service.upsertPreferences('view-1', { sortCode: 'name_asc' } as any, endUser);
