 ...
 */
export const { seeds: READ_MODELS_CONCEPT_SEEDS, ids: RM } = defineModuleConcepts('read_models', {
  // --- Tipo de objeto físico (read_model_definitions.object_type_concept_id) ---
  OBJECT_TYPE_VIEW: { code: 'RM_OBJ_VIEW', display: 'SQL view' },
  OBJECT_TYPE_MATERIALIZED_VIEW: { code: 'RM_OBJ_MVIEW', display: 'Materialized view' },

  // --- Modo de refresh de la definición (refresh_mode_concept_id) ---
  REFRESH_MODE_CONCURRENT: { code: 'RM_REFRESH_MODE_CONCURRENT', display: 'Concurrent refresh' },
  REFRESH_MODE_SCHEDULED: { code: 'RM_REFRESH_MODE_SCHEDULED', display: 'Scheduled refresh' },
  ...
  DEF_ACTIVE: { code: 'RM_DEF_ACTIVE', display: 'Read model definition active' },
  DEF_DEPRECATED: { code: 'RM_DEF_DEPRECATED', display: 'Read model definition deprecated' },
  DEF_RETIRED: { code: 'RM_DEF_RETIRED', display: 'Read model definition retired' },

  // --- Tipo de dependencia upstream (read_model_dependencies.dependency_type_concept_id) ---
  DEP_TYPE_TABLE: { code: 'RM_DEP_TABLE', display: 'Source table dependency' },
  ...
  REFRESH_TYPE_CONCURRENT: { code: 'RM_RUN_CONCURRENT', display: 'Concurrent refresh run' },
  REFRESH_TYPE_INCREMENTAL: { code: 'RM_RUN_INCREMENTAL', display: 'Incremental refresh run' },
  REFRESH_TYPE_FULL_BACKFILL: { code: 'RM_RUN_FULL_BACKFILL', display: 'Full backfill run' },
  REFRESH_TYPE_RECONCILE: { code: 'RM_RUN_RECONCILE', display: 'Reconcile run' },
  ...
  RESULT_DIVERGENCE_FOUND: { code: 'RM_RESULT_DIVERGENCE', display: 'Divergence found' },
  RESULT_REPAIRED: { code: 'RM_RESULT_REPAIRED', display: 'Divergence repaired' },

  // --- Tipo de portal (portal_surfaces.portal_type_concept_id) ---
  PORTAL_TYPE_INTERNAL: { code: 'RM_PORTAL_INTERNAL', display: 'Internal portal' },
  ...
  VIEW_TYPE_DASHBOARD: { code: 'RM_VIEW_DASHBOARD', display: 'Dashboard view' },
  ...
  ACTION_TYPE_NAVIGATE: { code: 'RM_ACTION_NAVIGATE', display: 'Navigate action' },
  ACTION_TYPE_MUTATION: { code: 'RM_ACTION_MUTATION', display: 'Mutation action' },
  ...
  STATE_TYPE_FORBIDDEN: { code: 'RM_STATE_FORBIDDEN', display: 'Forbidden state' },
  ...
  DENSITY_COMFORTABLE: { code: 'RM_DENSITY_COMFORTABLE', display: 'Comfortable density' },
});

/** Mapea el código de tipo de objeto (DTO) a su concept id. */
export const OBJECT_TYPE_CONCEPT_BY_CODE: Record<'VIEW' | 'MATERIALIZED_VIEW', string> = {
...
export const PORTAL_TYPE_CONCEPT_BY_CODE: Record<'INTERNAL' | 'PUBLIC', string> = {
  INTERNAL: RM.PORTAL_TYPE_INTERNAL,
  PUBLIC: RM.PORTAL_TYPE_PUBLIC,
};

/** Mapea el código de tipo de vista (DTO) a su concept id. */
export const VIEW_TYPE_CONCEPT_BY_CODE: Record<'TABLE' | 'DASHBOARD' | 'DETAIL', string> = {
...
export const ACTION_TYPE_CONCEPT_BY_CODE: Record<'NAVIGATE' | 'MUTATION', string> = {
...
export const DENSITY_CONCEPT_BY_CODE: Record<'COMPACT' | 'COMFORTABLE', string> = {
