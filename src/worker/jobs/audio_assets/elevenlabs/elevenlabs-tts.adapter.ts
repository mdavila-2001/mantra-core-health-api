...
export class ElevenLabsTtsAdapter implements TtsProviderPort {
  ...
  private readonly bulkhead = new Bulkhead({
    operation: 'tts.elevenlabs',
    maxConcurrent: this.env.maxConcurrency,
    maxQueued: 0,
  });
  private readonly rateGate = new ProviderRateGate(
    this.env.maxRequestsPerSecond,
  );
  ...
  constructor(
    ...
    this.circuit = new CircuitBreaker({
      ...
      openDurationMs: this.env.circuitOpenMs,
      maxOpenDurationMs: this.env.circuitOpenMs * 8,
      isFailure: (error) =>
        error instanceof TtsProviderError && error.retryable,
      onStateChange: (change) =>
        this.logger.warn(
          { ...change, component: 'tts.elevenlabs.circuit' },
          'ElevenLabs circuit state changed',
        ),
    });
  }
    ...
    while (true) {
      try {
        ...
        if (
          ...
          !error.retryable ||
          attempt >= this.config.httpMaxRetries
        )
          throw error;
        const delayMs =
          error.retryAfterMs ??
          Math.min(this.config.retryBaseMs * 2 ** attempt, 5000);
  ...
  async health(): Promise<{ configured: boolean; provider: string }> {
    return {
      configured: Boolean(this.config.apiKey && this.env.providerVoiceRef),
      provider: this.providerName,
    };
  }
}
