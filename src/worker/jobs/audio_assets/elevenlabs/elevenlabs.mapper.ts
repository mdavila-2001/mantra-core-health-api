...
export function mapElevenLabsError(error: unknown): TtsProviderError {
  ...
  if (status === 429) {
    return new TtsProviderRateLimitedError(
  ...
  if ([400, 404, 409, 422].includes(status)) {
    return new TtsProviderInvalidRequestError(
      `ElevenLabs rejected synthesis request (${status})`,
    );
  }
  if (status >= 500)
    return new TtsProviderUnavailableError(
      `ElevenLabs unavailable (${status})`,
    );
  return new TtsProviderInvalidRequestError(
...
function safeErrorText(data: unknown): string {
  ...
  if (data instanceof ArrayBuffer)
    return Buffer.from(data).toString('utf8').slice(0, 4096);
  try {
    return JSON.stringify(data).slice(0, 4096);
  } catch {
    return '';
  }
}
