...
4. **01 · Alta de usuarios (por tipo)** crea, en orden y encadenando ids por el entorno:
   organización en sus tres tipos (`PROVIDER`, `PAYER` con su aseguradora, `BROKER` con su
   corredora), profesional de salud con su matrícula, paciente público, paciente asistido con
   su activación, usuario estándar, administrador de seguridad, elevación a `SUPERADMIN`,
   perfil de paciente con la vinculación de su cuenta, perfil de profesional, y un rol de
   negocio con su asignación por tenant. Los 15 pasos se verifican contra la API real con
   `yarn postman:verify`.
