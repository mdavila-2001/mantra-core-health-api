...
**Actualización 2026-09-13 (autorizada y verificada):** el usuario confirmó que una cobertura declarada (`VERIFY_PENDING`) basta. `LinkedClaimAccessService.coverageForOrder` ya no exige `VERIFY_VERIFIED`; sigue exigiendo `COVERAGE_ACTIVE` y vigencia de cobertura/plan. El bloqueo anterior queda como antecedente.

Con esa corrección, el int-spec aislado (`test/integration/patient-coverage-copays.int-spec.ts`, `--runInBand` sobre `mantra-copays-pg`) pasa **8 de 8, exit 0, 91.719 s**: reclamo vinculado → adjudicación → EOB publicada → lectura del paciente, en aprobado / parcial / denegado / pendiente de publicación, más el aislamiento entre pacientes, prestadores y aseguradoras ajenos. **El recorrido financiero completo deja de estar pendiente.** Unitarias asociadas: 16 suites y 261 pruebas de `src/modules/insurance`, y 7 suites con 113 de `src/modules/pharmacy_inventory`, todas exit 0; `yarn typecheck` exit 0 después de incorporar `dev`.

Llegar al final del recorrido destapó **dos defectos preexistentes de farmacia**, ajenos a esta subtarea, corregidos en el camino: (1) `uq_pharmacy_product_prices_pharmacy_price_list_id_vers_7ebb36be` versionaba por lista y no por producto — cualquier lista con dos productos moría con `23505` al dar de alta el segundo (modelo #22, patch `2026-09-14_v4213_pharmacy_price_version_scope.sql`); y (2) `LedgerRepository.nextSequence` calculaba `max()+1` con SQL crudo sin ver los asientos que `append` dejaba sin flushear, así que un pedido de dos líneas pedía dos veces la misma secuencia.
