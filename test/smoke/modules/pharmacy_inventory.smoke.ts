...
export const PHARMACY_INVENTORY_SMOKE: SmokeCase[] = [
  ...
  {
    module: 'PharmacyInventory',
    endpoint: 'POST /pharmacy/:siteId/inventory-locations',
    name: 'happy: ubicación',
    method: 'post',
    path: (c) => `/pharmacy/${c.vars.pharmacySiteId}/inventory-locations`,
    body: (c) => ({
      code: `LOC-${c.u}`,
      name: 'Estante A',
    ...
    expectedStatus: 201,
    capture: (b, c) => {
      c.vars.pinvLocationId = String(b.id);
    },
  },
  {
    module: 'PharmacyInventory',
  ...
  {
    module: 'PharmacyInventory',
    endpoint: 'POST /pharmacy/:pharmacyId/suppliers',
    name: 'happy: proveedor',
    method: 'post',
    path: (c) => `/pharmacy/${c.vars.pharmacyId}/suppliers`,
    body: (c) => ({ supplierTenantId: c.tenantId, supplierCode: `SUP-${c.u}` }),
    expectedStatus: 201,
    capture: (b, c) => {
      c.vars.pinvSupplierId = String(b.id);
    },
  },
  {
    module: 'PharmacyInventory',
    endpoint: 'POST /pharmacy/:pharmacyId/purchase-orders',
    name: 'límite: farmacia inexistente',
    method: 'post',
    path: () => `/pharmacy/${UUID_ABSENT}/purchase-orders`,
    body: (c) => ({ supplierId: c.vars.pinvSupplierId }),
    expectedStatus: 404,
  },
  // dispensación: depende de lote/inventario → se cubre 401
  {
    module: 'PharmacyInventory',
    endpoint: 'POST /pharmacy/:pharmacyId/dispensations',
    name: 'límite: sin auth (dispensación requiere lote)',
    method: 'post',
    path: (c) => `/pharmacy/${c.vars.pharmacyId}/dispensations`,
    auth: false,
    body: () => ({}),
    expectedStatus: 401,
  },
