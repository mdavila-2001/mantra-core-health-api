...
Agrupa los componentes relacionados con **alovida** y mantiene cohesionada esta responsabilidad del sistema.
...
| --- | --- |
| `coverage-report.mjs` | Informe estático de cobertura (`yarn alovida:coverage`). |
| `guardrails.mjs` | Guardrails bloqueantes de CI (`yarn alovida:guardrails`). |
...
| `exercise-clinical-personas.mjs` | Recorre el flujo de cada actor clínico **con su propio rol** (`yarn alovida:personas`). Falla si aparece un 5xx o un 4xx sin `code` del catálogo. |
