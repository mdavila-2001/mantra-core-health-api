...
function mapType(vaultType) {
  ...
  if (t === 'smallint')
    return { opts: `columnType: 'smallint'`, tsType: 'number' };
  ...
  if (t.startsWith('numeric'))
    return { opts: `columnType: '${t}'`, tsType: 'string' };
  if (t === 'double')
    return { opts: `columnType: 'double precision'`, tsType: 'number' };
  if (t === 'timestamptz' || t === 'datetime')
    return { opts: `columnType: 'timestamptz'`, tsType: 'Date' };
  if (t === 'date') return { opts: `columnType: 'date'`, tsType: 'string' };
  if (t === 'time') return { opts: `columnType: 'time'`, tsType: 'string' };
  if (t === 'jsonb' || t === 'json')
    return { opts: `type: 'json', columnType: 'jsonb'`, tsType: 'unknown' };
  if (t === 'inet') return { opts: `columnType: 'inet'`, tsType: 'string' };
  if (t === 'bytea') return { opts: `columnType: 'bytea'`, tsType: 'Buffer' };
  if (t === 'interval')
    return { opts: `columnType: 'interval'`, tsType: 'string' };
  if (t === 'vector') return { opts: `columnType: 'vector'`, tsType: 'string' };
  if (t.startsWith('char'))
    return { opts: `columnType: '${t}'`, tsType: 'string' };
  if (t === 'technical_data_type')
    return {
      opts: `columnType: '"terminology"."technical_data_type"'`,
      tsType: 'string',
    };
  return { opts: `columnType: '${t}'`, tsType: 'string' };
}
...
for (const [key, v] of entities) {
  if (SKIP_TIPOS.has(v.tipo) || SKIP_SCHEMAS.has(v.schema) || v.referenceOnly)
    continue;
  ...
  if (!mod) {
    console.warn(`sin módulo destino: ${key}`);
    continue;
  }
  ...
  const pkFields = v.fields.filter((f) =>
    /PK|TIME_KEY|SERIES_KEY|PARTITION_KEY/.test(f.marker),
  );
  ...
  for (const f of v.fields) {
    ...
    const parts = [];
    if (f.name !== camel(f.name) && !(isPk && singleUuidPk))
      parts.push(`fieldName: '${f.name}'`);
  ...
  created.push({
    key,
    mod,
    file: file.replace(SRC_MODULES + '/', ''),
    cols: v.fields.length,
  });
}
...
console.log(
  created.map((c) => `  ${c.key} (${c.cols} col) -> ${c.file}`).join('\n'),
);
