...
for (const [key, e] of ts)
  managed.set(key, new Set(e.props.map((p) => p.fieldName)));

// ---------------------------------------------------------------- schemas ---
const schemas = new Map(); // schema -> {module, moduleDir, tables}
for (const [key, e] of ts) {
  const cur = schemas.get(e.schema) ?? {
    schema: e.schema,
    moduleDir: e.module,
    moduleNumber: null,
    tables: 0,
...
for (const fk of foreignKeys) {
  ...
  if (pk.length !== 1 || !managed.get(dst).has(pk[0])) {
    fkSkipped++;
    continue;
  }
...
for (const [key, defs] of indexSets) {
  ...
  for (const d of defs) {
    ...
    if (!d.cols.length || !d.cols.every((c) => cols.has(columnName(c)))) {
      idxSkipped++;
      continue;
    }
      ...
      writeFileSync(
        ...
        [
          ...
          header(
            schema,
            chunk.length,
            chunks.length > 1 ? `${i + 1}/${chunks.length}` : null,
          ),
...
function writeBarrel(dir, barrel, typeName, constName, doc) {
  const imports = barrel
    .map((b) => `import { ${b.name} } from './${b.file}';`)
    .join('\n');
  const groups = new Map();
  for (const b of barrel)
    groups.set(b.schema, [...(groups.get(b.schema) ?? []), b.name]);
...
// ------------------------------------------------------ schemas.catalog -----
const schemaRows = [...schemas.values()].sort((a, b) =>
  a.schema.localeCompare(b.schema),
);
writeFileSync(
  join(ROOT, 'schemas.catalog.ts'),
  [
    ...
    ...schemaRows.map(
      (s) =>
        `  ['${s.schema}', ${s.moduleNumber ?? 'null'}, '${s.moduleDir}', ${s.tables}],`,
    ),
    `];`,
    '',
  ].join('\n'),
